...
public class WsdlGroundingBuilder {
	...
	public WSDLTranslator translate(javax.wsdl.Definition wsdlDef,
			AbstractService aService) throws Exception {
		...
		if (OWLS2WSDLSettings.getInstance().getProperty("CHANGE_BASE")
				.equals("yes")) {
			String owls_base = OWLS2WSDLSettings.getInstance().getProperty(
					"BASE");
		...
		if (OWLS2WSDLSettings.getInstance().getProperty("CHANGE_TNS")
			...
			String localFilename = aService.getLocalFilename().substring(0,
					index)
					+ ".wsdl";
		...
		if (OWLS2WSDLSettings.getInstance().getProperty("CHANGE_WSDLPATH")
				.equals("yes")) {
			String wsdl_basepath = OWLS2WSDLSettings.getInstance().getProperty(
					"WSDLPATH");
			index = aService.getLocalFilename().lastIndexOf(".");
			String localFilename = aService.getLocalFilename().substring(0,
					index)
		...
		System.out.println("[WSDLTranslator] getDocumentBaseURI: "
				+ wsdlDef.getDocumentBaseURI());
		System.out.println("[WSDLTranslator] getTargetNamespace: "
				+ wsdlDef.getTargetNamespace());
		System.out.println("[WSDLTranslator] getQName          : "
				+ wsdlDef.getQName().toString());
		System.out.println("[WSDLTranslator] ServiceName       : "
				+ aService.getName());
		System.out.println("[WSDLTranslator] ServiceDescr.     : "
