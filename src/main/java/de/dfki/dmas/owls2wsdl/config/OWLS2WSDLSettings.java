...
public class OWLS2WSDLSettings {
	...
	private OWLS2WSDLSettings() {
		...
		this.settingsFile = new File(this.homepath + File.separator
				+ "owls2wsdl.properties");
	...
	private void initProperties() {
		...
		this.properties.setProperty("logfile", new File(this.homepath
				+ "/owls2wsdl.log").getPath());
		...
		this.properties.setProperty("projectslot", ""); // Idee: durch Komma
														// getrennte liste
		this.properties.setProperty("EXPORT_PATH", new File(this.homepath
				+ "/owls2wsdl").getPath());
		...
		this.properties.setProperty("TNS_BASEPATH",
				"http://dmas.dfki.de/axis/services/");
		this.properties.setProperty("CHANGE_WSDLPATH", "yes");
		this.properties.setProperty("WSDLPATH", "http://127.0.0.1/grounding/");
		// f�r GUI zus�tzlich recent_service_dirs und recent_ontology_dirs
	}
	...
	public void loadPropertyFile() {
		...
		if (!this.settingsFile.exists()) {
			...
			try {
				this.properties.storeToXML(new FileOutputStream(
						this.settingsFile), "OWLS2WSDL properties", "UTF-8"); // java
																				// 1.5
				// this.properties.store(new
				// FileOutputStream(this.settingsFile), "OWLS2WSDL properties");
				// // java .1.4
		...
		try {
			...
			this.properties.loadFromXML(new FileInputStream(this.settingsFile)); // java
																					// 1.5
	...
	public void savePropertyFile() {
		try {
			...
			this.properties.storeToXML(new FileOutputStream(this.settingsFile),
					"OWLS2WSDL properties", "UTF-8"); // java 1.5
			// this.properties.store(new FileOutputStream(this.settingsFile),
			// "OWLS2WSDL properties"); // java 1.4
			System.out.println("[i] properties in "
					+ this.settingsFile.getName() + " saved.");
		} catch (java.io.IOException ioe) {
			System.err.println("Can't save " + this.settingsFile.toURI());
		}
	}
	...
	public boolean containsKey(String key) {
		Enumeration<? extends Object> names = this.properties.propertyNames();
		boolean found = false;
	...
	public Vector<String> getRecentValues(String key) {
	...
	public void addValue(String key, String value) {
		if (this.containsKey(key)) {
			...
			if (temp.contains(",")) {
				...
				for (int i = 0; i < templist.length; i++) {
					if (templist[i].trim().equals(value)) {
						found = true;
					}
				}
				if (!found) {
	...
	public static void main(String[] args) {
		...
		// OWLS2WSDLSettings.getInstance().printProperties(System.out);
		// Vector vals =
		// OWLS2WSDLSettings.getInstance().getRecentValues("recent_service_dirs");
