...
public class HandbookEnvBuilder {
	 ...
	 * 
	 * @param hb_urlString
	 *            handbooks url (web address)
	 * @param picsString
	 *            comma seperated list of pictures for handbook
	 */
	public HandbookEnvBuilder(String hb_urlString, String picsString)
		...
		this.handbookPicsDir = new File(applPath + File.separator + "handbook"
				+ File.separator + "pics");
		this.handbookIndex = new File(applPath + File.separator + "handbook"
				+ File.separator + "index.html");

		System.out.println("[HandbookEnvBuilder] handbook directory: "
				+ handbookDir);
		if (handbookDir.mkdir()) {
			System.out
					.println("[HandbookEnvBuilder] new handbook directory created");
		}
		if (handbookPicsDir.mkdir()) {
			System.out
					.println("[HandbookEnvBuilder] new pics directory for handbook created");
		}

		// BufferedReader in = new BufferedReader( new
		// InputStreamReader(this.hb_index.openStream()));
		...
		for (int i = 0; i < picNames.length; i++) {
			...
			if (new File(this.handbookPicsDir + File.separator + picNames[i])
					.exists()) {
				System.out.println("[HandbookEnvBuilder] "
						+ this.handbookPicsDir + File.separator + picNames[i]
						+ " exits!");
			} else {
				download(this.hb_pics + picNames[i], this.handbookPicsDir
	...
	public URL getHandbookIndexURL() throws MalformedURLException {
		return this.handbookIndex.toURI().toURL();
	}
