...
public class TranslationUtils {
	 ...
	 * 
	 * @param list
	 *            of AbstractService objects
	 * @return common directory name
	 * @author Oliver Fourman
	 */
	public static String findCommonRoot(Vector<AbstractService> directoryList)
			throws java.lang.ArrayIndexOutOfBoundsException {
		Vector<String> buffer = new Vector<String>();
		for (Iterator<AbstractService> it = directoryList.iterator(); it
				.hasNext();) {
			URI uri = URI.create(it.next().getFilename());
			String path = uri.getPath().substring(
	...
	public static String findCommonRootInStringList(Vector<String> buffer)
			throws java.lang.ArrayIndexOutOfBoundsException {
		LongestCommonSubstring lcsApp = new LongestCommonSubstring();
		String common = buffer.get(0).toString();
		for (int i = 0; i < buffer.size(); i++) {
