...
/**
 * This class reads an XSD file to extract XPATHs. Most of the code is simple.
 * However, a Stack is used to track which types of elements are visited so that
 * the program won't infinitely recurse when a container can contain itself. For
 * example, <foo><bar><foo/></bar></foo>
 */
public class XpathFromSchema {

	private static int numXpaths = 0;
	private static Stack<String> visitedTypes = new Stack<String>();
	...
	/**
	 * there may be some situations where the starting xpath is actually a
	 * prefix because the starting node can be different from the root node. In
	 * either case, allowing an xpath as a parameter provides flexibility for
	 * unforeseen needs.
	 */
	private static void dump(final String xpath, final ElementDecl elementDecl) {
		...
		if (typeReference.getName() != null
				&& visitedTypes.contains(typeReference.getName())) {
			// The type is already in the stack, therefore if we were to
			// continue we would infinitely recurse.
			System.out.println("The type (" + typeReference.getName()
					+ ") element (" + elementDecl.getName()
			...
			if (typeReference.isComplexType()) {
				...
				Enumeration<AttributeDecl> attributes = ct.getAttributeDecls();
				while (attributes.hasMoreElements()) {
					AttributeDecl attributeDecl = attributes.nextElement();
	...
	/**
	 * I have no idea what a group is, but a little experimentation showed the
	...
	public static void dumpGroup(String xpath, final Group group) {
		...
		while (particles.hasMoreElements()) {
			...
			if (o instanceof Group) {
				...
				System.out.println("[dumpGroup] ***** Unknown particle type: "
						+ o.getClass().getName());
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			// since this is a demonstration program, I select the topmost
			// element to
			// start with.
