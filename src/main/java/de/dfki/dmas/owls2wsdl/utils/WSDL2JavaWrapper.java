...
public class WSDL2JavaWrapper implements Runnable {
	...
	public static final String genops[] = { GENOP_AXIS_ONLY, GENOP_AXIS_CASTOR };

	private String genid = null;

	public static String[] AXISLIBS = { "activation.jar", "axis-ant.jar",
			"axis.jar", "commons-discovery.jar", "commons-logging.jar",
			"dsn.jar", "imap.jar", "jaxrpc.jar", "junit-3.8.1.jar",
			"log4j-1.2.8.jar", "mail.jar", "mailapi.jar", "pop3.jar",
			"saaj.jar", "smtp.jar", "wsdl4j-1.5.1.jar" };
	...
	public WSDL2JavaWrapper() {
		...
		if (OWLS2WSDLSettings.getInstance().containsKey("APP_PATH")) {
			this.emitterPath = OWLS2WSDLSettings.getInstance().getProperty(
					"APPL_PATH");
		} else {
			this.emitterPath = System.getProperty("user.home") + File.separator
					+ "owls2wsdl";
		}
		System.out.println("[WSDL2JavaWrapper] APPL_PATH: " + this.emitterPath);
		System.out.println("[WSDL2JavaWrapper] user.home >>> "
		...
		if (tf.exists() && tf.isDirectory()) {
			System.out.println("[WSDL2JavaWrapper] export path: "
					+ tf.getAbsolutePath());
		}
	}
	...
	public void options() throws Exception {
		...
		System.out.println("[WSDL2JavaWrapper] wsdlFile       : "
				+ wsdlFile.getPath());
		System.out.println("[WSDL2JavaWrapper] wsdlFile Parent: "
				+ wsdlFile.getParentFile().getAbsolutePath());
		// System.setProperty("user.dir",
		// wsdlFile.getParentFile().getAbsolutePath());

		String[] args = { "-v", "-D", "-s", "-pde.dfki.dmas.owls2wsdl.service",
				"-o" + this.emitterPath, "-t", "-dRequest", wsdlFile.getPath() };
		...
		// API error solved:
		// main method of WSDL2Java class executes with FileException using
		// whitespaces in cmd flags
		...
		// " org.apache.axis.wsdl.WSDL2Java " + argsString;
	...
	public void run() {
		...
		if (p != null && s != null && this.genid.equals(GENOP_AXIS_ONLY)) {
			...
			String wsdlFilename = s.getFilename().substring(idx1 + 1, idx2)
					+ ".wsdl";
			File wsdlFile = new File(this.emitterPath + File.separator
					+ wsdlFilename);
			...
			System.out
					.println("[WSDL2JavaWrapper] getReformatedServicename4Gen: ("
							+ reformatedServiceName + ")");
			String pkgname = "de.dfki.dmas.owls2wsdl.service."
					+ reformatedServiceName;

			// System.setProperty("user.dir", this.emitterPath);

			String[] args = { "-v", "-D", "-s", "-p" + pkgname,
					"-o" + this.emitterPath, "-t", "-dRequest",
					wsdlFile.getPath() };
			...
			try {
				...
				&& this.genid.equals(GENOP_AXIS_CASTOR)) {
			JOptionPane
					.showMessageDialog(
							this.mainWindowRef,
							"Option "
							...
							"WSDL2Java", JOptionPane.INFORMATION_MESSAGE);
		} else {
			if (p == null) {
