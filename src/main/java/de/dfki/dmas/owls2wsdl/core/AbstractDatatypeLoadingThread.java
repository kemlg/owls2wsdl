...
/**
 * Class to load persistent datatype information. Try to improve loading
 * persitent datatype file.
 * 
 * @author cosmic
 */
public class AbstractDatatypeLoadingThread extends Thread {
	...
	public void run() {
		try {
			...
			mapping.loadMapping(this.getClass().getResource(mapping_file)
					.toString());
			FileReader reader = new FileReader(this._file);

			Unmarshaller unmarshaller = new Unmarshaller(
					AbstractDatatypeKBData.class);
			unmarshaller.setMapping(mapping);

			AbstractDatatypeKBData data = (AbstractDatatypeKBData) unmarshaller
					.unmarshal(reader);
			if (OWLS2WSDLSettings.getInstance()
				...
				if (RuntimeModel.getInstance().getProject() != null) {
					int dependencyCount = RuntimeModel.getInstance()
							.getProject().getServiceDependencyTypes().size();
					int missingTypeCount = RuntimeModel.getInstance()
							.getProject().getServiceMissingTypes().size();
					for (;;) {
						System.out
								.println("[.] load DTs, count of dependency types: "
										+ dependencyCount);
						System.out
								.println("[.] load DTs, count of missing    types: "
										+ missingTypeCount);
						AbstractDatatypeKB
								.getInstance()
								...
								.importDatatypes(
										...
										RuntimeModel.getInstance().getProject()
												.getServiceMissingTypes());
						RuntimeModel.getInstance().getProject()
								.determineAllDependecyTypes();
						if (RuntimeModel.getInstance().getProject()
								.getServiceDependencyTypes().size() > dependencyCount) {
							dependencyCount = RuntimeModel.getInstance()
									.getProject().getServiceDependencyTypes()
									.size();
						} else {
							break;
						}
			...
			if (_listenerRef != null) {
				...
				RuntimeModel.getInstance().setStatusAndNotify("RUNTIME",
						...
						"Imported from: " + this._file.getName());
