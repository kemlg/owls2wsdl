 ...
 * Die Klasse wurde n�tig, weil Castor beim Mappen einen �ffentlichen -
 * Konstrukor ben�tigt und AbstractDatatypeKB als Singleton keinen anbietet.
...
public class AbstractDatatypeKBData implements java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7094551043355811312L;
	private HashMap<String, AbstractDatatype> _registeredDatatypes = new HashMap<String, AbstractDatatype>();
	private HashMap<String, Exception> _erroneousDatatypes = new HashMap<String, Exception>();
	private HashMap<String, AbstractDatatype> _metaDatatypes = new HashMap<String, AbstractDatatype>();
	private Date _modificationTime;
	private Vector<String> _ontologyURIs = new Vector<String>();

	private Vector<String> _missingDataTypes = new Vector<String>(); // no
																		// persistency

	/** Creates a new instance of NewClass */
	public AbstractDatatypeKBData() {
		System.out.println("[C] AbstractDatatypeKBData "
				+ this.getClass().cast(this)); // java 1.5
		this._registeredDatatypes = new HashMap<String, AbstractDatatype>();
		this._erroneousDatatypes = new HashMap<String, Exception>();
		this._metaDatatypes = new HashMap<String, AbstractDatatype>();
		this._modificationTime = new java.util.Date();
		this._ontologyURIs = new Vector<String>();
	}

	protected void finalize() {
		System.out.println("[D] AbstractDatatypeKBData "
				+ this.getClass().cast(this)); // java 1.5
	}

	public HashMap<String, AbstractDatatype> getRegisteredDatatypes() {
		return this._registeredDatatypes;
	}

	public HashMap<String, Exception> getErroneousDatatypes() {
		return this._erroneousDatatypes;
	}

	public HashMap<String, AbstractDatatype> getMetaDatatypes() {
		return this._metaDatatypes;
	}
	...
	public Vector<String> getMissingDatatypeURIs() {
		return this._missingDataTypes;
	}

	public void setRegisteredDatatypes(
			HashMap<String, AbstractDatatype> registeredTypes) {
		this._registeredDatatypes = registeredTypes;
	}

	public void setErroneousDatatypes(
			HashMap<String, Exception> erroneousDatatypes) {
		this._erroneousDatatypes = erroneousDatatypes;
	}

	public void setMetaDatatypes(HashMap<String, AbstractDatatype> metaDatatypes) {
	...
	public void setOntologyURIs(Vector<String> importedOntologyURIs) {
		this._ontologyURIs = importedOntologyURIs;
	}

	public void addOntologyURIs(Vector<String> importedOntologyURIs) {
		for (Iterator<String> it = importedOntologyURIs.iterator(); it
				.hasNext();) {
	...
	public void importDatatypes(AbstractDatatypeKBData data) {
		...
		for (Iterator<String> it = data.getOntologyURIs().iterator(); it
				.hasNext();) {
	...
	public void importDatatypes(AbstractDatatypeKBData data,
			Vector<String> greenlist) {
		System.out.println("[i] import referenced datatypes");
		boolean CHANGE_FLAG = false;
		for (Iterator<String> it = data.getRegisteredDatatypes().keySet()
				.iterator(); it.hasNext();) {
	...
	public boolean containsKey(String key) {
		return this._registeredDatatypes.containsKey(key)
				|| this._metaDatatypes.containsKey(key);
	}
	...
	public AbstractDatatype get(String key) {
		...
		// }
		return this._registeredDatatypes.get(key);
	}

	/**
	 * Remove AbstractDatatype entry from registered datatypes including all
	 * references to metatypes.
	 * 
	 * @param key
	 *            of AbstractDatatype (URL)
	 */
	...
	public AbstractDatatype getMeta(String key) {
		return this._metaDatatypes.get(key);
	}

	private Vector<String> collectParents(String uri) {
			...
			for (int i = 0; i < this.get(uri).getParentList().size(); i++) {
				...
				if (this.containsKey(parent)) {
					if (!this.get(parent).getParentList().isEmpty()) {
						Vector<String> temp2 = this.collectParents(parent);
	...
	public HashMap<String, String> collectAllDependencyTypes(String uri) {
		HashMap<String, String> loadedDependencyTypes = new HashMap<String, String>();

		Vector<String> parentList = this.collectParents(uri);
		for (int i = 0; i < parentList.size(); i++) {
			if (!this.containsKey(parentList.get(i).toString())) {
				if (!loadedDependencyTypes.keySet().contains(
						parentList.get(i).toString())) {
					loadedDependencyTypes
							.put(parentList.get(i).toString(), "0");
			...
			AbstractDatatype parentType = this
					.get(parentList.get(i).toString());
			loadedDependencyTypes.put(parentType.getUrl(), "1");

			for (Iterator<String> it = parentType.getIntersectionList()
					.iterator(); it.hasNext();) {
				String intersectionString = it.next().toString();
				if (!loadedDependencyTypes.keySet()
						.contains(intersectionString)) {
			...
			for (Iterator<String> it = parentType.getTypeList().iterator(); it
					.hasNext();) {
			...
			for (Iterator<AbstractDatatypeElement> it = parentType
					.getProperties().iterator(); it.hasNext();) {
				AbstractDatatypeElement element = it.next();
	...
	public void printRegisteredDatatypes() {
		for (Iterator<AbstractDatatype> it = _registeredDatatypes.values()
				.iterator(); it.hasNext();) {
			System.out.println("AbstractDatatype: " + it.next().getUrl());
		}
	}
