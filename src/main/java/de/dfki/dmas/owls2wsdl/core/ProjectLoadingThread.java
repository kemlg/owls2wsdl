...
public class ProjectLoadingThread extends Thread {
	...
	public void run() {
		...
		try {
			System.out.println("[ProjectLoadingThread] LOAD PROJECT FILE: "
					+ this._file.getPath());

			Project p = ProjectManager.getInstance().loadPersistentProject(
					this._file);
			if (p != null) {
				// lock.lock();
				...
				RuntimeModel.getInstance().setRuntimeAndNotify(
						RuntimeModel.PROJECT_LOADED,
						"project " + this._file.getName() + " loaded");
				RuntimeModel.getInstance().setRuntimeAndNotify(
						RuntimeModel.SERVICE_MODEL_CHANGED);
				RuntimeModel.getInstance().setRuntimeAndNotify(
						RuntimeModel.DATATYPE_MODEL_CHANGED); // status table
			}

			if (_listenerRef != null) {
				this._listenerRef.getMainWindowRef().setTitle(
						"OWLS2WSDL Tool - " + p.getProjectName());
	...
	public static void main(String[] args) {
		...
		for (Iterator<String> it = p.getServiceDependencyTypes().iterator(); it
				.hasNext();) {
		...
		for (Iterator<String> it = p.getServiceMissingTypes().iterator(); it
				.hasNext();) {
			System.out.println(".) " + it.next().toString());
		}

		// Vector undefinedList =
		// RuntimeModel.getInstance().getProject().collectUndefinedDatatypes();
