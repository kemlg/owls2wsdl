...
public class XsdSchemaGenerator {
    ...
    private HashSet hack_02;      // For ABSTRACT elements reinit. hack01 (more element declarations)
    private HashSet renamedTypes; // temp. registry to lookup renamed types: +"Type"
     ...
     * @param schemaID             Necessary in castors schema model
     * @param useHierarchyPattern  Flag to enable the hierarchy pattern (xsd pattern)
     * @param depth                Indicates the depth, that should be used to inherit elements from owl superclasses and intersections
     */
    public XsdSchemaGenerator(  String schemaID,
    ...
    /** Creates a new instance of XsdSchemaGenerator 
     * @param schemaID             Necessary in castors schema model
     * @param useHierarchyPattern  Flag to switch on the hierarchy pattern (xsd pattern)
     * @param depth                Indicates the depth, that should be used to inherit elements from owl superclasses and intersections
     */
    public XsdSchemaGenerator(  String schemaID,
                                boolean useHierarchyPattern,
    ...
    /**
     * Set flag to enable meta information about owl language constructs (logic).
     */
    ...
    public void appendToSchema(AbstractDatatype curtype) throws Exception
    {
        ...
        if(this.schema.getElementDecl(curtype.getLocalName())!=null) {
            System.out.println("[xsdgen] appendToSchema, remove abstract attribute");
            this.schema.getElementDecl(curtype.getLocalName()).setAbstract(false);
        }
        this.toXSD(curtype);
    }
       
    private SimpleType addSimpleType(AbstractDatatype curtype, String elemName) throws SchemaException
    {
        String simpleTypeName = null;
        if(curtype.getLocalName().equals(elemName)) {       // in der Regel bei Parametertypen
        ...
        if(this.schema.getSimpleType(curtype.getLocalName())!=null) {
            if(this.renamedTypes.contains(curtype)) {
                this.schema.getSimpleType(curtype.getLocalName()).setName(curtype.getLocalName()+"Type");
                System.out.println("[xsdgen] addSimpleType, rename and use existing type "+simpleTypeName);
                return this.schema.getSimpleType(curtype.getLocalName()+"Type");
            }
            else {
                System.out.println("[xsdgen] addSimpleType, use existing type "+simpleTypeName);
                return this.schema.getSimpleType(curtype.getLocalName());
            }
        }
        else if(this.schema.getSimpleType(curtype.getLocalName()+"Type")!=null) {
            if(this.renamedTypes.contains(curtype)) {
                System.out.println("[xsdgen] addSimpleType, use existing renamed type "+simpleTypeName);
                return this.schema.getSimpleType(curtype.getLocalName()+"Type");
            }
        }
        else {
            System.out.println("[xsdgen] addSimpleType, build and add type "+curtype);
        }                
        
        String xsdTypeString = curtype.determineXsdType(this.xsdInheritance, this.DEFAULT_XSD);
        ...
        SimpleType baseType = stf.getBuiltInType( this.extractLocalName(xsdTypeString) );
        SimpleType simpleAnyType = stf.getBuiltInType( this.schema.getBuiltInTypeName( SimpleTypesFactory.ANYURI_TYPE ) );
        ...
        if(curtype.getIndividualRange().isEmpty()) {
            System.out.println("[i] Build SIMPLE TYPE ("+simpleTypeName+") only");
            simpleType = this.schema.createSimpleType(simpleTypeName, baseType);            
        }
        else  {
            System.out.println("[i] Build SIMPLE TYPE ("+simpleTypeName+") with restriction values.");
            System.out.println("    Count of rangeTypes: "+curtype.getIndividualRangeTypes().size());                                
            
            if(curtype.getIndividualRangeTypes().size() == 1) 
            {                    
                String dependencyTypeURL = curtype.getIndividualRangeTypes().get(0).toString();
                if(curtype.getUrl().equals(dependencyTypeURL)) {
                    System.out.println("[i] type equals enumeration type, set same baseType");
                    simpleType = this.schema.createSimpleType(simpleTypeName, baseType);
                }
                else {
                    System.out.println("[i] Restriction enumeration type different from simple type");
                    if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().containsKey(dependencyTypeURL)) {
                        AbstractDatatype dependencyAbstractType = AbstractDatatypeKB.getInstance().data.get(dependencyTypeURL);                    
                        String dtypeString = dependencyAbstractType.determineXsdType(this.xsdInheritance, this.DEFAULT_XSD);
                        if(dtypeString.equals(this.DEFAULT_XSD)) {
                            dtypeString = curtype.determineXsdType(this.xsdInheritance, this.DEFAULT_XSD);
                        }
                        baseType = stf.getBuiltInType( this.extractLocalName(dtypeString) );
                        simpleType = this.schema.createSimpleType(simpleTypeName, baseType);
                    }
                ...
                simpleType = this.schema.createSimpleType(simpleTypeName, simpleAnyType);
            }

//                SimpleType simpleTypeWithValues = this.schema.createSimpleType(simpleTypeName+"Data", simpleType);

            for(Iterator it=curtype.getIndividualRange().keySet().iterator(); it.hasNext(); ) {
...
//                if(this.schema.getSimpleType(simpleTypeWithValues.getName()) == null) {
...
//        }
        ...
        if(this.schema.getSimpleType(simpleType.getName()) == null) {
            ...
            System.out.println("[i] add simpleType "+simpleType.getName()+" (1)");
        }
        return simpleType;
    } 
    
    private SimpleType addSimpleDATAType(AbstractDatatype curtype) throws SchemaException
    {   
        ...
        if(!curtype.getLocalName().equals(curtype.getBaseName())) {
            if(this.schema.getSimpleType(curtype.getBaseName()+"DATA") == null) {
                AbstractDatatype abstractBaseType = AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(curtype.getUrl());
                System.out.println("[i] have to add SimpleDATAType for not yet existing basetype: "+abstractBaseType.toString());
                this.addSimpleDATAType(abstractBaseType);                
            }
            System.out.println("[i] getSimpleType: "+curtype.getBaseName()+"DATA");
            baseType = this.schema.getSimpleType(curtype.getBaseName()+"DATA");
        }
        
        System.out.println("[i] Build SIMPLE TYPE ("+curtype.getLocalName()+") with restriction values.");
        System.out.println("    Count of rangeTypes: "+curtype.getIndividualRangeTypes().size());                                
        ...
        {                    
            ...
            if(curtype.getUrl().equals(dependencyTypeURL)) {
                System.out.println("[i] type equals enumeration type, set same baseType");
                simpleDATAType = this.schema.createSimpleType(curtype.getLocalName()+"DATA", baseType);
            }
            else {
                System.out.println("[i] Restriction enumeration type different from simple type");
                if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().containsKey(dependencyTypeURL)) {
            ...
            System.out.println("[?] different type in restriction set found!");
            simpleDATAType = this.schema.createSimpleType(curtype.getLocalName()+"DATA", simpleAnyType);
        }
        ...
        if(this.schema.getSimpleType(simpleDATAType.getName()) == null) {
            ...
            System.out.println("[i] add simpleType "+simpleDATAType.getName()+" (1data)");
        }
    ...
    {
        SimpleTypesFactory stf = new SimpleTypesFactory();
        SimpleType anyType = stf.getBuiltInType(this.schema.getBuiltInTypeName(SimpleTypesFactory.ANYURI_TYPE));
        SimpleType nonNegativeIntegerType = stf.getBuiltInType(this.schema.getBuiltInTypeName(SimpleTypesFactory.NON_NEGATIVE_INTEGER_TYPE));
        
        SimpleType OntProperty    = this.schema.createSimpleType("OntProperty", anyType);
        SimpleType Cardinality    = this.schema.createSimpleType("Cardinality", nonNegativeIntegerType);
        SimpleType MinCardinality = this.schema.createSimpleType("MinCardinality", nonNegativeIntegerType);
        SimpleType MaxCardinality = this.schema.createSimpleType("MaxCardinality", nonNegativeIntegerType);        
        SimpleType DataLiteral    = this.schema.createSimpleType("DataLiteral", anyType);
        ...
        ElementDecl oneOfDataLiteral = new ElementDecl(this.schema, "oneOfDataLiteral");
        ...
        ComplexType AllValuesFrom = this.schema.createComplexType("AllValuesFrom");
        ...
        ComplexType SomeValuesFrom = this.schema.createComplexType("SomeValuesFrom");
        ...
        ElementDecl dataLiteralRangeElement = new ElementDecl(this.schema, "range");        
        ...
        // === 
        ElementDecl allValuesFromElement = new ElementDecl(this.schema, "allValuesFrom");
        allValuesFromElement.setType(AllValuesFrom);
        ElementDecl someValuesFromElement = new ElementDecl(this.schema, "someValuesFrom");
        ...
        ElementDecl cardinalityElement = new ElementDecl(this.schema, "cardinality");
        cardinalityElement.setType(Cardinality);
        ElementDecl minCardinalityElement = new ElementDecl(this.schema, "minCardinality");
        minCardinalityElement.setType(MinCardinality);
        ElementDecl maxCardinalityElement = new ElementDecl(this.schema, "maxCardinality");
        maxCardinalityElement.setType(MaxCardinality);
        
        ComplexType RestrictionComponent = this.schema.createComplexType("RestrictionComponent");
        ...
        ElementDecl onPropertyElement = new ElementDecl(this.schema, "onProperty");        
        onPropertyElement.setType(OntProperty);
        ElementDecl restrictedByElement = new ElementDecl(this.schema, "restrictedBy");
    ...
    {        
        ...
        // collect documentation from previous used types (Bugfix, noch nicht in Castor 1.1!)
        ...
        while(annotations.hasMoreElements()) {
            ...
            while(docs.hasMoreElements()) {
                ...
                System.out.println("Annotation /COLLECT DOC: "+doc.getSource());
        ...
        System.out.println("Annotation /NEW DOC: "+currentSchemaDocumentation.getSource());
        currentSchemaAnnotation.addDocumentation(currentSchemaDocumentation);                    
        this.schema.addAnnotation(currentSchemaAnnotation);        
    }
    
    private Group addChoiceGroup(AbstractDatatype curtype, Group group) throws SchemaException
    {
        System.out.println("[xsdgen] ComplexType, CHOICE FALL-1: "+curtype.getUrl());
        // choice
        ...
        if(this.schema.getComplexType(curtype.getLocalName())!=null) {                
            if(this.schema.getComplexType(curtype.getLocalName()).isRestricted()) {
                System.out.println("RESTRICTED TYPE FOUND: "+this.schema.getComplexType(curtype.getLocalName()).getBaseType().getName());
            }

            if(this.schema.getComplexType(curtype.getLocalName()).isRedefined()) {
                System.out.println("REDEFINED TYPE FOUND: "+this.schema.getComplexType(curtype.getLocalName()).getReferenceId());
            }
        }
        ...
        SimpleTypesFactory stf = new SimpleTypesFactory();
        SimpleType simpleStringType = stf.getBuiltInType( this.schema.getBuiltInTypeName( SimpleTypesFactory.STRING_TYPE ) );
    ...
    public void deleteObsoleteTypesFromSchema() 
    {
        ...
        for(Iterator<ComplexType> it=this.obsoleteComplexTypes.iterator(); it.hasNext(); ) {
            ...
            if(this.schema.removeComplexType(obsoleteType)) {
                System.out.println("[xsdgen] obsolete ComplexType ("+obsoleteTypeName+") removed.");
            }
        }
    }
                    ...
                    while(grpEnumeration.hasMoreElements()) {
                        ...
                        if(grpObj instanceof org.exolab.castor.xml.schema.ElementDecl) {
                            ...
                            if(elem.getType()==null) {
                                System.out.println("[xsdgen]   Type of element ("+elem.getName()+") is null.");
                            }
                            else {
                                String typeName = elem.getType().getName();
                                if(elem.getType().isComplexType() && this.obsoleteComplexTypes.contains(elem.getType())) {
                                    elem.setType(this.schema.getComplexType(typeName+"Type"));
                                    System.out.println("[xsdgen] "+elem.getName()+" type changed to ("+typeName+"Type)");
                                }
                            }
                        }
                        else {
                            System.out.println("[?] unknown GRP ENUM-OBJ: "+grpObj.getClass().getName());
                        }                                
                    }
                }
            }
    ...
    {       
        ...
        SimpleTypesFactory stf = new SimpleTypesFactory();
        //SimpleType anySimpleType = stf.getBuiltInType( this.schema.getBuiltInTypeName( SimpleTypesFactory.ANYSIMPLETYPE_TYPE ) );
        SimpleType anyURIType = stf.getBuiltInType( this.schema.getBuiltInTypeName( SimpleTypesFactory.ANYURI_TYPE ) );
...
//        SimpleType simpleThingType = this.schema.createSimpleType("Thing", defaultType);        
        ...
        System.out.println("[i] add SimpleThingType "+simpleThingType.getName()+" ("+simpleThingType.getBaseType().getName()+")");
        return simpleThingType;
    }
    
    private ComplexType addComplexType(AbstractDatatype curtype, String typeName) throws SchemaException 
    {
        System.out.println("[xsdgen] addComplexType "+curtype.getLocalName()+", "+typeName);
        ...
        System.out.println("[xsdgen] add ComplexType ("+typeName+") for "+curtype.getUrl());
        ...
        if(!curtype.getIndividualRange().isEmpty()) {
            System.out.println("[i] add Choice + Sequence Group for "+curtype.getLocalName());
            Group choicegroup = this.addChoiceGroup(curtype, group);
            complexType.addGroup(choicegroup);                
        }
        else {
            System.out.println("[i] add Sequence Group for "+curtype.getLocalName());
            complexType.addGroup(group);
        }
        
        this.schema.addComplexType(complexType); // An der Stelle ins Schema-Modell setzen, um eine Rekursionsabbruch zu erzielen.

 /** PROPERTIES 
  */    
        System.out.println("\n*********** " +
                "Properties for "+curtype.getLocalName()+" *********\n");

        Iterator pit = curtype.getProperties(depth).iterator();
        while(pit.hasNext()) {
            AbstractDatatypeElement elem = (AbstractDatatypeElement)pit.next();
            System.out.println("=========================================");
            System.out.println("[xsdgen] AbstractDatatypeElement: "+elem.toString());
            elem.printData();                                
            System.out.println("=========================================");
            ...
            if(!elem.getRestrictions().isEmpty() && this.printAnnotations)  {
                for(Iterator it=elem.getRestrictions().iterator(); it.hasNext(); ) {
                    AbstractDatatypeRestrictionElement resElem = (AbstractDatatypeRestrictionElement)it.next();
                    Annotation annotation = new Annotation();
                    Documentation doc = new Documentation();
                    doc.setSource("OWL Restriction "+resElem.getRestrictionType()+"="+resElem.getRestrictionValue());
            ...
            if(elem.isPrimitive()) {
                System.out.println("[xsdgen] "+elem.getLocalName()+" Type: "+elem.getType());
                SimpleTypesFactory stf = new SimpleTypesFactory();
                SimpleType baseType = null;                    
                baseType = stf.getBuiltInType( this.extractLocalName(elem.getType()) );
                if(baseType == null) {
                    baseType = stf.getBuiltInType( this.schema.getBuiltInTypeName( SimpleTypesFactory.ANYURI_TYPE ) );
                }                    
                ...
                if(elem.getRestrictedRange().size() == 1) {
                    subEl.setFixedValue(elem.getRestrictedRange().get(0).toString());
                }
            }
            else if(elem.getOwlSource().equals("DATATYPE") || elem.getOwlSource().equals("RESTRICTION-ON-DATATYPE") || elem.getOwlSource().equals("RESTRICTION")) 
            {
                ...
                // TESTE, OB XSDTYPE GESETZT, ODER �BER HIERARCHY GEERBT WERDEN KANN. -> SIMPLE TYPE
                // WENN KEIN XSDTYPE GESETZT UND PROPERTIES �BER DEPTH ANGEZOGEN WERDEN -> COMPLEX TYPE
                ...
                if(baseType == null) {
                    ...
                    if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().containsKey(elem.getType())) {
                        System.out.println("DATATYPE PROPERTY WITH COMPLEX DATATYPE FOUND.");
                        if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(elem.getType()).getXsdType()!=null) {
                            String xsdtype = AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(elem.getType()).getXsdType();
                            System.out.println("SEMI-AUTOMATIC SET TYPE FOUND: "+xsdtype);
                            baseType = stf.getBuiltInType( this.extractLocalName(xsdtype) );
                        }
                    }
                }                    
                ...
                if(elem.getRestrictedRange().size() == 1) {
                    ...
                    if( elem.getName().contains("Cardinality") ||
                        ...
                        baseType = stf.getBuiltInType(stf.getBuiltInTypeName(SimpleTypesFactory.NON_NEGATIVE_INTEGER_TYPE));
                    }
                ...
                // RESTRICTED SIMPLE TYPE
                    ...
                    SimpleType simpleType = this.schema.createSimpleType(elem.getLocalName()+"Type", baseType);                    
                    for(Iterator it=elem.getRestrictedRange().iterator(); it.hasNext(); ) {
                    ...
                    this.schema.addSimpleType(simpleType);
                    System.out.println("[i] add simpleType "+simpleType.getName()+" (2)");
            ...
            else if(elem.getOwlSource().equals("UNION") || elem.getOwlSource().equals("INTERSECTION")) 
            {
                if(this.printOwlInformation) {
                    SimpleTypesFactory stf = new SimpleTypesFactory();
                    System.out.println("[xsdgen] [meta] subEl.setName: "+elem.getLocalName()+" and META-Type: "+elem.getType());
                    ...
                    if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().getMeta(elem.getType()).getProperties().isEmpty()) {
                        System.out.println("[xsdgen] [meta] SimpleType");
                        SimpleType simpleType = stf.getBuiltInType( this.schema.getBuiltInTypeName( SimpleTypesFactory.ANYURI_TYPE ) );
                        subEl.setType(simpleType);
                    }
                    else {                       
                        ...
                        if(elem.getOwlSource().equals("UNION")) {
                            if(this.schema.getComplexType("Union") == null) {
                                ComplexType unionType = this.schema.createComplexType("Union");
                            ...
                            if(this.schema.getComplexType("Intersection") == null) {
                                ComplexType intersectionType = this.schema.createComplexType("Intersection");
                                intersectionType.setAbstract(true);
                                this.schema.addComplexType(intersectionType);
                            }
                        }
                        else {
                            System.out.println("[xsdgen] [meta] can't create base type");
                        }
                        
                        //
                        // build Meta ComplexType (Intersection or Union with elements)
                        //
                        AbstractDatatype aMetaType = AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().getMeta(elem.getType());
                        System.out.println("[xsdgen] [meta] add ComplexType "+aMetaType.getLocalName());                        
                        
                        ComplexType subMetaType = this.addComplexType(aMetaType, aMetaType.getLocalName());
                        
                        // set base type
                        if(elem.getOwlSource().equals("UNION")) {
                            subMetaType.setComplexContent(true);
                            subMetaType.setDerivationMethod(SchemaNames.EXTENSION); // hier werden ja Elemente hinzugef�gt.
                        ...
                        if(elem.getRestrictedRange().isEmpty()) {
                            System.out.println("META has no RestrictedRange, element has specified value(s) and no more subproperties ");
                        }
                        else {
                ...
                // Complex Type with 2 elements: onProperty(OntProperty), restrictedBy(RestrictionComponent)
                ...
                AbstractDatatype metatype = AbstractDatatypeKB.getInstance().data.getMeta(elem.getType());
                Group restrictionGroup = new Group();
...
//                    restrictionGroup.addElementDecl(isRestrictionElement);
                ...
                {
                    for(Iterator metaIt=metatype.getProperties().iterator();metaIt.hasNext(); ) {
                        AbstractDatatypeElement atypeElem = (AbstractDatatypeElement)metaIt.next();
                        if(atypeElem.getOwlSource().equals("ONPROPERTY")) {                                
                            //SimpleType onPropertyType = this.schema.createSimpleType(this.extractLocalName(atypeElem.getType()), anyType);                                
                            ElementDecl metaEl = new ElementDecl(this.schema, "onProperty");
                            metaEl.setType(anyType);                                
                            metaEl.setFixedValue(atypeElem.getName());
                            restrictionGroup.addElementDecl(metaEl);
                        }
                        else {
                            ElementDecl metaEl = new ElementDecl(this.schema, atypeElem.getLocalName());

                            if(atypeElem.getType().equals("Cardinality")) {
                                metaEl.setType(this.schema.getSimpleType("Cardinality"));
                            }
                            else {                                    
                                if(this.schema.getSimpleType(atypeElem.getLocalTypeName()) != null) {
                                    metaEl.setType(this.schema.getSimpleType(atypeElem.getLocalTypeName()));
                                }
                                else if(this.schema.getComplexType(atypeElem.getLocalTypeName()) != null) {
                                    metaEl.setType(this.schema.getComplexType(atypeElem.getLocalTypeName()));
                                }
                                else {
                                    SimpleType componentType = this.schema.createSimpleType(atypeElem.getLocalTypeName(), anyType);
                                    metaEl.setType(componentType);
                                }
                            }

                            if(!atypeElem.getRestrictedRange().isEmpty()) {
                                metaEl.setFixedValue(atypeElem.getRestrictedRange().get(0).toString());
                            }
                            restrictionGroup.addElementDecl(metaEl);                                
//                                    for(Iterator facetIt=atypeElem.getRestrictedRange().iterator(); facetIt.hasNext(); ) {                                    
//                                        componentType.addFacet(new Facet(Facet.ENUMERATION, facetIt.next().toString()));
//                                    }
                        }
                    }

                    ComplexType subMetaCT = this.schema.createComplexType(metatype.getLocalName());                        
                ...
                subEl.setName(elem.getLocalName());
                subEl.setType(this.schema.getComplexType(metatype.getLocalName()));
            }
            else {
                // OBJECT.PROPERTY -> COMPLEX.TYPE
                // INTERSECTION-TYPE, INTERSECTION-CLASS

                System.out.println("[i] [complex] subEl.setName: "+elem.getLocalName()+" and Type: "+elem.getLocalTypeName());
                subEl.setName(elem.getLocalName());                    

                if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().containsKey(elem.getType()))    
                {
                    System.out.println("REGISTERED DATATYPE FROM KB: "+elem.getType());
                    ...
                    if(AbstractDatatypeKB.getInstance().data.get(elem.getType()).getProperties(this.depth).isEmpty()) 
                    {                            
                        //searching for meta information in restriction list
                        for(Iterator it=elem.getRestrictions().iterator(); it.hasNext(); ) {
                            AbstractDatatypeRestrictionElement restriction = (AbstractDatatypeRestrictionElement)it.next();
                            if(restriction.getRestrictionType().equals("isFunctional")) {
                                subEl.setMinOccurs(0);
                                subEl.setMaxOccurs(1);
                                if(this.printAnnotations) {
                                    Annotation annotation = new Annotation();
                                    Documentation doc = new Documentation();
                                    doc.setSource("isFunctional"); //, inherited by "+restriction.getInheritedBy());                                    
                                    annotation.addDocumentation(doc);
                                    subEl.addAnnotation(annotation);
                                }
                            }
                            if(restriction.getRestrictionType().equals("isTransitive")){
                                if(this.printAnnotations) {
                                    Annotation annotation = new Annotation();
                                    Documentation doc = new Documentation();
                        ...
                        //
                        // Check if we must create an element specific SimpleType (element restrictions!)
                        //
                        if(!elem.getRestrictedRange().isEmpty()) {
                            // element has specific value(s) and no more subproperties 
                            // ~> do not run recursion! build SimpleType with element restrictions
                            System.out.println("[xsdgen] SimpleType with restricted range: "+curtype.getLocalName()+" ("+curtype.getUrl()+")");

                            String restrictedSimpleTypeName = curtype.getLocalName()+elem.getLocalTypeName(); //elem.getLocalName()+elem.getLocalTypeName(); // StEmilionSpecificType
                            System.out.println("[xsdgen] New SimpleType name: "+restrictedSimpleTypeName);
...
// Zweiter Ansatz: baseType der zu restriktierenden Klasse �bernehmen                                                                
                            if(baseType == null) {
                            ...
                            simpleType = this.schema.createSimpleType(restrictedSimpleTypeName, baseType);                                

                            for(Iterator it=elem.getRestrictedRange().iterator(); it.hasNext(); ) {                                        
                                //System.out.println("[XSD GEN] RANGE: "+elem.getRestrictedRange().get(i).toString());
                                //String value = this.extractLocalName(elem.getRestrictedRange().get(i).toString());
                                String rangeURI = it.next().toString();
                                System.out.println("[xsdgen] RANGE: "+rangeURI);
                                String value = this.extractLocalName(rangeURI);
                                simpleType.addFacet(new Facet(Facet.ENUMERATION, value));
                            }
                            ...
                            if(this.schema.getSimpleType(simpleType.getName())==null) {
                                this.schema.addSimpleType(simpleType);
                                System.out.println("[i] addSimpleType "+simpleType.getName()+" (3)");
                                /*
                                if(this.useHierarchyPattern) {
                                    ElementDecl baseTypeElDec = new ElementDecl(this.schema, baseType.getName());
                                    baseTypeElDec.setAbstract(true);
                                    this.schema.addElementDecl(baseTypeElDec);
                                    ElementDecl simpleTypeElDec = new ElementDecl(this.schema, simpleType.getName());
                                    simpleTypeElDec.setSubstitutionGroup(baseType.getName());                                        
                            ...
                            // *) Bemerkung: vorher war ein Rekursionslauf n�tig, jetzt Abfrage �ber KB.                            
                        ...
                        System.out.println("[i] ComplexType "+elem.getLocalName()+" ("+elem.getType()+")");

                        AbstractDatatype type = AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(elem.getType());
                        System.out.println("TYPE                     : "+type.toString());
                        System.out.println("ANZAHL INDIVUDALS VON TYP: "+type.getIndividualRange().keySet().size());
                        System.out.println("RESTRICTED RANGE         : "+elem.getRestrictedRange().size());

                        ComplexType subCT = null;
                        System.out.println("[xsdgen] add new ComplexType ("+type.getLocalName()+") for sub element.");
                        if(type.getLocalName().equals(subEl.getName())) {
                            subCT = this.addComplexType(type, type.getLocalName()+"Type");
                        }
                        else {
                            subCT = this.addComplexType(type, type.getLocalName());
                        }
                        ...
                        if(elem.getRestrictedRange().isEmpty()) {
                            ...
                            AbstractDatatype subtype = new AbstractDatatype(type); // CopyConstructor

                            // subCT is basetype
                            System.out.println("[xsdgen] ComplexType with restricted range: "+curtype.getLocalName()+" ("+curtype.getUrl()+")");

                            String subTypeName = curtype.getLocalName()+type.getLocalName();
                            System.out.println("[i] Neuer Subtype: "+subTypeName);                                
                            subtype.setLocalName(subTypeName);
                            subtype.removeAllRanges();
                            for(int i=0; i<elem.getRestrictedRange().size(); i++) {
                                subtype.addIndividualRange(elem.getRestrictedRange().get(i).toString(), elem.getType());
                            }
                            
                            ComplexType subComplexType = this.addComplexType(subtype, subtype.getLocalName());
                            subComplexType.setBase(type.getLocalName());
                            subComplexType.setBaseType(subCT);
                            subComplexType.setDerivationMethod(SchemaNames.RESTRICTION);
                            System.out.println("[xsdgen] NEW restricted Sub ComplexType for element added: "+subComplexType.getName());
                            subEl.setType(subComplexType);
                        }

                        if(elem.getMinOccurs() == -1 && elem.getMaxOccurs() == -1 && !elem.getOwlSource().equals("INTERSECTION-TYPE")) {
                            subEl.setMaxOccurs(Particle.UNBOUNDED);
                        }
                    }
                }
                else {
                    System.out.println("NOT IN KB: "+elem.getType()+" Build THING ...");
                    SimpleType simpleType = null;

                    if(!elem.getRestrictedRange().isEmpty()) {
                        // element has specific value(s) and no more subproperties 
                        // ~> build SimpleType Thing with element restrictions!
                        System.out.println("[XSD GEN] SimpleType for Restriction.");

                        SimpleTypesFactory stf = new SimpleTypesFactory();                    
                        SimpleType baseType = stf.getBuiltInType(this.DEFAULT_XSD);
                        
                        if(elem.getLocalTypeName().equals("Thing")) {
                            simpleType = this.schema.createSimpleType(elem.getLocalTypeName()+Integer.toString(thingNo++), baseType);
                        }
                        else {
                            simpleType = this.schema.createSimpleType(elem.getLocalTypeName(), baseType);
                        }
                        for(Iterator it=elem.getRestrictedRange().iterator(); it.hasNext(); ) {                                
                            //System.out.println("[XSD GEN] RANGE: "+elem.getRestrictedRange().get(i).toString());
                            //String value = this.extractLocalName(elem.getRestrictedRange().get(i).toString());
                        ...
                        if(this.schema.getSimpleType(simpleType.getName())==null) {
                            this.schema.addSimpleType(simpleType);
                            System.out.println("[i] add simpleType "+simpleType.getName()+" (4)");
                        }
                    }
                    else {                            
                        if(elem.getType().equals("http://www.w3.org/2002/07/owl#Thing")) {
                            System.out.println("[xsdgen] explicit defined thing found.");
...
//                subEl.setMaxOccurs(elem.getMaxOccurs());
            ...
            if(group.getElementDecl(subEl.getName()) == null) {
                if(elem.getOwlSource().equals("UNION") || elem.getOwlSource().equals("INTERSECTION")) {
                    if(this.printOwlInformation) {
                        System.out.println("[xsdgen] ADD META Element "+subEl.getName()+" ("+subEl.getType().getName()+") to group.");
                        group.addElementDecl(subEl);
                    }
                }
                else {
                    if(subEl.getType()!=null) {
                        System.out.println("[xsdgen] ADD Element "+subEl.getName()+" ("+subEl.getType().getName()+") to group.");
                    }
                    else {
                        System.out.println("[xsdgen] ADD Element "+subEl.getName()+" (without type declaration!) to group.");
                    }                        
                    group.addElementDecl(subEl);
                }
            }
            else {
                System.out.println("[xsdgen] Element "+subEl.getName()+" ("+subEl.getType().getName()+") already in group.");
            }
        }  // END WHILE
        
        //complexType.setName(typeName);
        //this.schema.addComplexType(complexType);        // nach oben verschieben wegen Rekursionsabbruch!
        ...
        if(useHierarchyPattern && curtype.getParentList().size()>0) 
        {
            ...
            ElementDecl baseElement = this.schema.getElementDecl(abstractElementBaseName);
            if(baseElement == null) {
                baseElement = new ElementDecl(this.schema, abstractElementBaseName);
            ...
            if(parentURI.equals(URI_THING) && baseType==null) {
                System.out.println("[i] missing parent is Thing, add simple ThingType");
                baseType = this.addSimpleThingType();
                abstractElementBaseName = "Thing";
            }
            
            if(this.schema.getType(abstractElementBaseName+"Type")==null) {
                System.out.println("[HierarchyPattern] missing base type: "+abstractElementBaseName+"Type");
                System.out.println("[HierarchyPattern] parentURI: "+parentURI);
                if(AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().containsKey(parentURI)) {
                    AbstractDatatype abstractBaseType = AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(parentURI);                    
                    if(abstractBaseType.isComplexType(this.depth)) {
                        baseType = this.addComplexType(abstractBaseType, abstractBaseType.getLocalName()+"Type");
                    }
                    else {
                        baseType = this.addSimpleType(abstractBaseType, abstractBaseType.getLocalName()+"Type");
                    }                    
                }
            }
            else {
                baseType = this.schema.getType(abstractElementBaseName+"Type");
            }
            ...
            if(this.schema.getElementDecl(curtype.getLocalName()) != null) {
                this.schema.getElementDecl(curtype.getLocalName()).setSubstitutionGroup(abstractElementBaseName);
            }
     ...
     * @param curtype  The AbstractDatatype object that is used to build an element
     */
    ...
    {   
        ...
        System.out.println("[xsdgen] to XSD, property count: "+curtype.getProperties(this.depth).size());
        
        if(curtype.getProperties(this.depth).isEmpty()) //!curtype.hasElements(this.depth) )
        {   
            SimpleType simpleType = this.addSimpleType(curtype, curtype.getLocalName());
            ...
            this.addSchemaAnnotation("Translation (OWL2XSD-SimpleType) of "+curtype.getUrl());
        }
        else {            
            System.out.println("[xsdgen] add new ComplexType ("+curtype.getLocalName()+")Type for "+curtype.getUrl());            
            ComplexType complexType = this.addComplexType(curtype, curtype.getLocalName()+"Type");
            
            // correct ComplexTypes when changing Names (add. "Type") due composing schemas for WSDL.
            ComplexType complexSubType = this.schema.getComplexType(curtype.getLocalName());
            if(complexSubType != null) {                
                System.out.println("Loaded complexSubType Id:"+complexSubType.getId()+", Name: "+complexSubType.getName());                
                if(complexSubType.isTopLevel()) {
                    System.out.println("[xsdgen] complexSubType "+complexSubType+" is top-level type.");
                    
                    ElementDecl elem = (ElementDecl)complexSubType.getParent();
                    System.out.println("[xsdgen] complexSubType Parent: "+elem.getName());
...
//                System.out.println("[xsdgen] use existing ComplexType ("+curtype.getLocalName() +")Type for "+curtype.getUrl());
//                complexType = this.schema.getComplexType(curtype.getLocalName());
...
//            ComplexType complexType = this.schema.getComplexType(curtype.getLocalName());
//            if(complexType == null ) {
//                complexType = this.addComplexType(curtype, curtype.getLocalName());
//            }
            ...
            if(this.hack_01 == false) 
            {
                ...
                if(useHierarchyPattern) 
                {
                    ...
                    if(curtype.getIntersectionList().size() == 1) {
                        if(curtype.getIntersectionList().get(0).toString().equals(curtype.getUrl())) {
                            System.out.println("[xsdgen] not using HierarchyPattern, intersection with same type.");
                        }
                        else {
                            System.out.println("[xsdgen] use HierarchyPattern: getIntersectionList == 1");
                            
                            String abstractElementName = this.extractLocalName(curtype.getIntersectionList().get(0).toString());
                            ...
                            AbstractDatatype subtype = AbstractDatatypeKB.getInstance().data.get(curtype.getIntersectionList().get(0).toString());
                            
                            if(subtype.isComplexType(this.depth)) {
                                complexType.setComplexContent(true);
                                complexType.setDerivationMethod(SchemaNames.RESTRICTION);
                                complexType.setRestriction(true);
                                complexType.setBase(abstractElementName+"Type");
                                complexType.setBaseType(this.schema.getType(abstractElementName+"Type"));
                            }
                            else {
                                if(this.printAnnotations) {
                                    Annotation annotation = new Annotation();
                                    Documentation doc = new Documentation();
                                    doc.setSource("hasSimpleIntersection "+abstractElementName);
                                    annotation.addDocumentation(doc);                                
                                    complexType.addAnnotation(annotation);
                                }
                                System.out.println("[i] can't set ComplexContent (hierarchy pattern), parent is SimpleType");
                            }
                        ...
                        String parentURI  = curtype.getParentList().get(0).toString();
                        String abstractElementName = this.extractLocalName(parentURI);
                        System.out.println("[xsdgen] useHierarchyPattern: getParentList == 1, "+parentURI);
                        
                        ElementDecl parentElement = null;
                        if(this.schema.getElementDecl(abstractElementName) == null) {
                            parentElement = new ElementDecl(this.schema, abstractElementName);
                            parentElement.setAbstract(true);
                        }
                        else {
                            parentElement = this.schema.getElementDecl(abstractElementName);
                        }                        
                        ...
                        System.out.println("[xsdgen] build hierarchy ref for type: "+curtype.getUrl());
                        System.out.println("[xsdgen] found one parent type: "+parentURI);
                        ...
                        if(parentURI.equals(URI_THING)) {
                            ...
                            AbstractDatatype parentType 
                                    = AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(parentURI);
                        
                            if(parentType.isComplexType(this.depth)) {
                                //dependencies.add(parentType);
                                ComplexType complexBaseType = this.addComplexType(parentType, abstractElementName+"Type");
                                ...
                                SimpleType simpleBaseType = this.addSimpleType(parentType, abstractElementName+"Type");
                                parentElement.setType(simpleBaseType);                                
                                if(this.printAnnotations) {
                                    Annotation annotation = new Annotation();
                                    Documentation doc = new Documentation();
                                    doc.setSource("parent is SimpleType "+abstractElementName);
                                    annotation.addDocumentation(doc);                                
                                    complexType.addAnnotation(annotation);
                                }
                                System.out.println("[i] can't set ComplexContent (hierarchy pattern), parent is SimpleType");
                        ...
                        System.out.println("[e] more than one intersection/parent classes. use mixin pattern!");
                    }
                }                                                                
                
                // elDecl.setMaxOccurs(Particle.UNBOUNDED); // jedes Element muss mind. einmal vorkommen. Geht aber hier nicht wegen META Element.                                
                
                if(this.schema.getElementDecl(elDecl.getName())==null) {
                    this.schema.addElementDecl(elDecl);                    
                    this.addSchemaAnnotation("Translation (OWL2XSD-ComplexType) of "+curtype.getUrl());
                    System.out.println("[xsdgen] element "+elDecl.getName()+" added.");
                }
                else {
                    System.out.println("[xsdgen] element "+elDecl.getName()+" already in schema.");
                }
            }
            else {
                if(useHierarchyPattern) {
                     ...
                     */
                    if(curtype.getIntersectionList().size() == 1) {
                        this.hack_02.add(this.extractLocalName(curtype.getIntersectionList().get(0).toString()));
                    }
                    else if(curtype.getParentList().size() == 1) {
                        this.hack_02.add(this.extractLocalName(curtype.getParentList().get(0).toString()));
                    }
                    else if(curtype.getIntersectionList().size() > 1) {
                        System.out.println("[e] more than one intersection classes. use mixin pattern!");
                    }
                    else if(curtype.getParentList().size() > 1) {
                        System.out.println("[e] more than one parent classes. use mixin pattern!");
                    }
                    else {
                        System.out.println("[i] no intersection class available.");
                    }                
                    
                    if(this.hack_02.contains(curtype.getLocalName())) {         

                        complexType.setName(curtype.getLocalName()+"Type"); //25.1.2007  +"Type"); 
                        if(schema.getComplexType(complexType.getName()) == null) {
                            this.schema.addComplexType(complexType);
                            System.out.println("[xsdgen]-c1: "+complexType.getName());
                        }
                        else {
                            System.out.println("[xsdgen]-c1 "+complexType.getName()+" already in schema.");
                        }
                        ...
                        if(curtype.getIntersectionList().size() == 1) {
                            if(curtype.getIntersectionList().get(0).toString().equals(curtype.getUrl())) {
                                ...
                                System.out.println("     AbstractElement: "+abstractElementName);
                                AbstractDatatype supertype = AbstractDatatypeKB.getInstance().data.get(curtype.getIntersectionList().get(0).toString());
                            ...
                            System.out.println("[xsdgen] build hierarchy ref for type: "+curtype.getUrl());
                            System.out.println("[xsdgen] found parent type: "+parentURI);
                        ...
                        if(this.schema.getElementDecl(elDec.getName())==null) {
                            ...
                            this.addSchemaAnnotation("Translation (OWL2XSD-ComplexType) of "+curtype.getUrl());
                            System.out.println("[xsdgen] element "+elDec.getName()+" added.");
                        }
                        else {
                            System.out.println("[xsdgen] element "+elDec.getName()+" already in schema.");
                        }
                    }
                    else {
                        if(schema.getComplexType(complexType.getName()) == null) {
                            this.schema.addComplexType(complexType);
                            System.out.println("[xsdgen]-c2: "+complexType.getName());
                        }
                        else {
                            System.out.println("[xsdgen]-c2 "+complexType.getName()+" already in schema");
                        }
                    }
                }
                else {
                    ...
                    if(schema.getComplexType(complexType.getName()) == null) {
                        this.schema.addComplexType(complexType);
                        System.out.println("[xsdgen]-c3: "+complexType.getName());
                    }
                    else {
                        System.out.println("[xsdgen]-c3 "+complexType.getName()+" already in schema");
                    }
                }                
...
//                }  
            ...
            if(DEBUGFLAG) {
                ...
                while(allElDecls.hasMoreElements()) {
                    System.out.println("[xsdgen] ELEM: "+((ElementDecl)allElDecls.nextElement()).getName());
                }  
            ...
            // Abfrage: wenn schon ein Element im Schema, kann es nicht hinzugef�gt werden.
            // 
            for(int i=0; i<dependencies.size(); i++) {
                System.out.println("=======================================================================");
                System.out.println("REKURSION with "+((AbstractDatatype)dependencies.get(i)).getLocalName());
