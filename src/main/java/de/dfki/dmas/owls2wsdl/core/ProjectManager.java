...
public class ProjectManager {
	...
	public void mapProject(Project project, OutputStream out,
			boolean prettyprint) {
		try {
	...
	public Project loadPersistentProject(File file) {
		...
		try {
			...
			System.out.println("LOAD Mapping File: "
			...
			AbstractDatatypeKB.getInstance().setAbstractDatatypeKBData(
					project.getAbstractDatatypeKBData());
		} catch (Throwable t) {
			t.printStackTrace();
		}
		return project;
	...
	public static void main(String[] args) {
		...
		try {
			project = ProjectManager.getInstance().loadPersistentProject(
					new File("D:\\tmp\\Project_1.xml"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		System.out.println(">>> "
				+ project.getAbstractDatatypeKBData().getClass()
						.cast(project.getAbstractDatatypeKBData())); // java 1.5
	}
}
