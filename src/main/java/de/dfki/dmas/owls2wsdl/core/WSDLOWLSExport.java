...
public class WSDLOWLSExport implements Runnable {
	...
	private Vector<AbstractService> _selectedServices;
	...
	public WSDLOWLSExport() {
		this._selectedServices = new Vector<AbstractService>();
		this.exportDirectory = null;

		if (OWLS2WSDLSettings.getInstance().containsKey("EXPORT_PATH")) {
			exportDirectory = new File(OWLS2WSDLSettings.getInstance()
					.getProperty("EXPORT_PATH"));
			System.out.println("[WSDLOWLSExport] EXPORT_PATH: "
	...
	public void setSelectedServices(Vector<AbstractService> services) {
	...
	public void setExportDirectory(String path) {
		...
		if (newDirectory.exists() && newDirectory.isDirectory()) {
			...
			System.out.println("[WSDLOWLSExport] can't set EXPORT_PATH to "
	...
	// {
	// Vector<AbstractService> translatableServiceList = new
	// Vector<AbstractService>();
	// for(Iterator<AbstractService> it=this._selectedServices.iterator();
	// it.hasNext(); ) {
	...
	// this.common =
	// de.dfki.dmas.owls2wsdl.utils.TranslationUtils.findCommonRoot(translatableServiceList);
	// }
	// }
	// catch(Exception e) {
	...
	public void run() {
		...
		File reportFile = new File(exportDirectory.getPath() + File.separator
				+ "report-" + df.format(now) + ".txt");
		try {
			...
			reportWriter
					.write("================================================================================\n");
			reportWriter.write("=== EXPORT LOG " + df.format(now)
					+ " === Project: " + this._project.getProjectName() + "\n");
			reportWriter
					.write("================================================================================\n");
			for (Iterator<String> it = this._project
					.getAbstractDatatypeKBData().getOntologyURIs().iterator(); it
					.hasNext();) {
				reportWriter.write("=== Parsed ontology: "
						+ it.next().toString() + "\n");
			}
			reportWriter.write("=== Project contents:\n");
			reportWriter.write("===   "
					+ this._project.getAbstractDatatypeKBData()
							.getRegisteredDatatypes().size()
					+ " registered datatypes\n");
			reportWriter.write("===   1.0 services: "
					+ this._project.getAbstractServiceCollection().getCount10()
					+ "\n");
			reportWriter.write("===   1.1 services: "
					+ this._project.getAbstractServiceCollection().getCount11()
					+ "\n");
			reportWriter.write("===   1.2 services: "
					+ this._project.getAbstractServiceCollection().getCount12()
					+ "\n");
			reportWriter.write("=== Settings:\n");
			reportWriter.write("===   Recursion depth           : "
					+ this._project.getElementDepth() + "\n");
			reportWriter.write("===   Default XSD type          : "
					+ this._project.getDefaultXsdType() + "\n");
			reportWriter.write("===   Type inheritance behaviour: "
					+ this._project.getTypeInheritanceBehaviour() + "\n");
			if (OWLS2WSDLSettings.getInstance().getProperty("xsdgen")
					.equals("hierarchy")) {
				reportWriter
						.write("===   Use specialization pattern (aka hierarchy pattern /xfront)\n");
			}
			if (OWLS2WSDLSettings.getInstance().getProperty("owlinfo")
					.equals("yes")) {
				reportWriter
						.write("===   Add additional information about owl definition to enhance WSDL matching\n");
			}
			reportWriter
					.write("================================================================================\n");
			reportWriter.close();
			...
			int counter = 0;
			for (Iterator<AbstractService> it = this._selectedServices
					.iterator(); it.hasNext();) {
				counter++;
				AbstractService aService = it.next();

				reportWriter = new FileWriter(reportFile, true);
				reportWriter.write("Service (" + counter + ") "
						+ aService.getName() + " (Vers.: "
						+ aService.getVersion() + ")\n");
				if (this._guiRef != null) {
					this._guiRef.appendLogMsg("Service (" + counter + ") "
							+ aService.getName() + " (Vers.: "
							+ aService.getVersion() + ")\n");
				}
				...
				if (aService.istranslatable()) {
					...
					// RuntimeModel.getInstance().getProject().buildXsdSchemaGenerator();
					XsdSchemaGenerator xsdgen = this._project
							.buildXsdSchemaGenerator();

					int idx1 = aService.getFilename().lastIndexOf("/");
					int idx2 = aService.getFilename().lastIndexOf(".");

					String wsdlFilename = aService.getFilename().substring(
							idx1 + 1, idx2)
							+ ".wsdl";
					...
					// String path =
					// uri.getPath().substring(uri.getPath().indexOf(":")+1);
					// int index = path.indexOf(this.common);
					// String path2File =
					// path.substring(index+this.common.length());
					// this.exportLog.append(exportdir.getAbsolutePath()+"/"+path2File+"\n");
					// }

					try {
						javax.wsdl.Definition wsdlDef = WSDLBuilder
								.getInstance()
								.buildDefinition(aService, xsdgen);

						// == WSDL File (WSDL2OWL-S)
						if (this.EXPORT_WSDL) {
							File wsdlFile = new File(exportDirectory.getPath()
									+ File.separator + wsdlFilename);
							System.out.println("[wsdl] " + wsdlFile.toString());
							if (wsdlFile.createNewFile()) {
								FileOutputStream outStream = new FileOutputStream(
										wsdlFile);
								WSDLBuilder.getInstance().printSchema(wsdlDef,
										outStream);
								outStream.close();
								if (this._guiRef != null) {
									this._guiRef.appendLogMsg("\t"
											+ exportDirectory.toString()
											+ File.separator + wsdlFilename
											+ "\n");
								}
								...
								reportWriter.write("Service (" + counter + ") "
										+ aService.getName() + " (Vers.: "
										+ aService.getVersion() + ")\n");
								reportWriter.write("\t" + wsdlFile.toString()
										+ " already exists.\n");
							}
						}
						...
						if (this.EXPORT_OWLS) {
							...
							String owlsFilename = aService.getFilename()
									.substring(idx1 + 1, idx2) + ".owls";
							System.out.println("[OWLS] " + owlsFilename);

							// String wsdlPath =
							// OWLS2WSDLSettings.getInstance().getProperty("WSDLPATH");
							...
							WSDLTranslator t = groundingBuilder.translate(
									wsdlDef, aService);

							File owlsFile = new File(exportDirectory.getPath()
									+ File.separator + owlsFilename);
							...
							if (owlsFile.createNewFile()) {
								...
								outStream.close();
								System.out.println("[owls] Saved to "
										+ owlsFile.toURI());
								if (this._guiRef != null) {
									this._guiRef.appendLogMsg("\t"
											+ exportDirectory.toString()
								...
								reportWriter.write("Service (" + counter + ") "
										+ aService.getName() + " (Vers.: "
										+ aService.getVersion() + ")\n");
								reportWriter.write("\t" + owlsFile.toString()
										+ " already exists.\n");
								reportWriter.close();
							}
						}
					} catch (javax.wsdl.WSDLException wsdlExc) {
						System.err.println("[e] WSDLException: "
								+ wsdlExc.getMessage());
					} catch (java.lang.Exception exc) {
						System.err
								.println("[e] Exception: " + exc.getMessage());
						exc.printStackTrace();
					}
				} else {
			...
			reportWriter
					.write("================================================================================\n");
			reportWriter
					.write("=== summary for selected services ===  [T]:translatable, [N]:nontranslatable\n");
			reportWriter
					.write("================================================================================\n");
			for (Iterator<AbstractService> it = this._selectedServices
					.iterator(); it.hasNext();) {
