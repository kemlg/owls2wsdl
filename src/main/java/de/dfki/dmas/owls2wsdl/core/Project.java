...
public class Project implements java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8316104390401664506L;
	...
	private Vector<String> _serviceDependencyTypes; // all types for loaded
													// services
	...
	public Project() {
		...
		this._defaultXsdType = DEFAULT_PROJECT_XSDTYPE;
		// this._ontologyList = new Vector();
		this._serviceDependencyTypes = new Vector<String>();
		this._serviceMissingTypes = new Vector<String>();
	}
	...
	public void setElementDepth(int depth) {
		this._elementDepth = depth;
	}

	public void setTypeInheritanceBehaviour(String behaviour) {
		this._typeInheritanceBehaviour = behaviour;
	}

	public void setAbstractServiceCollection(
			AbstractServiceCollection serviceCollection) {
		this._serviceCollection = serviceCollection;
	}

	public void setAbstractDatatypeKBData(
			AbstractDatatypeKBData abstractDatatypeKBData) {
		AbstractDatatypeKB.getInstance().setAbstractDatatypeKBData(
				abstractDatatypeKBData);
	}
	...
	public String getDefaultXsdType() {
		return this._defaultXsdType;
	}
	...
	public String getTypeInheritanceBehaviour() {
		return this._typeInheritanceBehaviour;
	}
	...
	public XsdSchemaGenerator buildXsdSchemaGenerator() {
		...
		XsdSchemaGenerator xsdgen = new XsdSchemaGenerator("WSDL",
				useHierarchyPattern, this.getElementDepth(),
				this.getTypeInheritanceBehaviour(), this.getDefaultXsdType());
	...
	public Vector<String> getServiceDependencyTypes() {
		return this._serviceDependencyTypes;
	}

	public Vector<String> getServiceMissingTypes() {
		return this._serviceMissingTypes;
	}

	public void importServices(String path) throws Exception {
		this._serviceCollection.addAbstractServiceList(serviceParser
				.parse(path));
	}
	...
	/**
	 * Check and search KB for undefined datatypes. That means, we collect
	 * parent, intersection and rdfs types, that are not already in KB to load
	 * them form persistent storage.
	 */
	public Vector<String> collectUndefinedDatatypes() {
		...
		for (Iterator<String> it = this._processCollection.getParameterTypes()
			...
			if (AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData()
					.containsKey(uri)) {
				System.out
						.println("[i] Already found URI, check dependencies: "
								+ uri);
			} else {
	...
	public void determineAllDependecyTypes() {
		...
		for (Iterator<String> paramIt = this._processCollection
			...
			if (this._serviceDependencyTypes.contains(uri)) {
				...
				HashMap<String, String> tempTypes = AbstractDatatypeKB
						.getInstance().getAbstractDatatypeKBData()
						.collectAllDependencyTypes(uri);
				for (Iterator<String> it = tempTypes.keySet().iterator(); it
						.hasNext();) {
	...
	public void removeUnreferencedDatatypes() {
		Vector<String> redlist = new Vector<String>();
		for (Iterator<String> it = AbstractDatatypeKB.getInstance()
				.getAbstractDatatypeKBData().getRegisteredDatatypes().keySet()
				.iterator(); it.hasNext();) {
		...
		for (Iterator<String> it = redlist.iterator(); it.hasNext();) {
			String key = it.next().toString();
			AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData()
					.removeRegisteredDatatype(key);
		}
	}
	...
	// for(Iterator
	// it=this._serviceCollection.getServiceCollection().iterator();
	// it.hasNext(); ) {
	...
	public static void main(String args[]) {
		...
		if (arg.equals("save")) {
			...
			AbstractDatatypeMapper.getInstance().loadAbstractDatatypeKB(
					"file:/D:/tmp/KB/KB_Student-MAP.xml");
			try {
		...
		// String service_1 =
		// "D:\\htw_kim\\thesis\\OWLS-MX\\owls-tc2\\domains\\1.1\\Weapon\\ballisticmissilegovernment_fundingrange_service.owls";
		// String service_2 =
		// "file:///D:/htw_kim/thesis/OWLS-MX/owls-tc2/queries/1.1/car_price_service.owls";
		// try {
		...
		//
		// project.setAbstractDatatypeKBData(
		// AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData() );
		//
