...
public class OWLS2WSDL {
	...
	private static void printDefaultHelpMessage() {
		...
		System.out
				.println(" FILE.owl     parse ontology file and generate persisten knowledgebase");
		System.out
				.println(" FILE.xml     load knowledgebase and generate XML Schema");
			...
			if (args[args.length - 1].toString().endsWith(".owl")) {
				...
				try {
					...
					if (cmdLine.hasOption("help")) {
						...
						formatter.printHelp("owls2wsdl [options] FILE.owl",
								options);
						System.exit(0);
					}
				} catch (ParseException exp) {
					// oops, something went wrong
					System.out.println("Error: Parsing failed, reason: "
							+ exp.getMessage());
					HelpFormatter formatter = new HelpFormatter();
				...
				if (args.length == 1) {
					...
					try {
						if (cmdLine.hasOption("kbdir")) {
							...
							if (new File(kbdirValue).isDirectory()) {
								...
								if (!cmdLine.hasOption("test")) {
									System.out.println("AUSGABE: " + kbdirValue
											+ File.separator + persistentName);
									FileOutputStream ausgabeStream = new FileOutputStream(
											kbdirValue + File.separator
													+ persistentName);
									AbstractDatatypeKB.getInstance()
											.marshallAsXML(ausgabeStream, true); // System.out);
								}
							}
						}
						...
						System.err.println("MalformedURLException: "
								+ murle.getMessage());
						System.err
								.println("Try something like: http://127.0.0.1/ontology/my_ontology.owl");
					} catch (Exception e) {
				...
				options.addOption("h", "hierarchy", false,
						"use hierarchy pattern");
				options.addOption("d", "depth", true, "set recursion depth");
				options.addOption("b", "behavior", true,
						"set inheritance bevavior");
				options.addOption("p", "primitive", true,
				...
				try {
					...
					if (cmdLine.hasOption("help")) {
						HelpFormatter formatter = new HelpFormatter();
						formatter.printHelp("owls2wsdl [options] FILE.xml",
								options);
						System.exit(0);
					}
				} catch (ParseException exp) {
				...
				if (args.length == 1) {
					HelpFormatter formatter = new HelpFormatter();
					formatter
							.printHelp("owls2wsdl [options] FILE.xml", options);
					System.exit(0);
				} else if (cmdLine.hasOption("keys")) {
					File f = new File(args[args.length - 1].toString());
					try {
						AbstractDatatypeMapper.getInstance()
								.loadAbstractDatatypeKB(f);
					} catch (Exception e) {
					...
					AbstractDatatypeKB.getInstance()
							...
							.printRegisteredDatatypes();
					System.exit(0);
				} else if (!cmdLine.hasOption("owlclass")) {
					HelpFormatter formatter = new HelpFormatter();
					formatter
							.printHelp(
									"owls2wsdl [options] FILE.xml",
									"",
									options,
					...
					if (!AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData()
							.containsKey(owlclassString)) {
						System.err.println("Error: Key " + owlclassString
					...
					if (cmdLine.hasOption("info")) {
						AbstractDatatypeKB.getInstance()
								.getAbstractDatatypeKBData()
								.get(owlclassString).printDatatype();
					}
					if (cmdLine.hasOption("xsd")) {
						...
						if (cmdLine.hasOption("d")) {
							depth = Integer.parseInt(cmdLine
									.getOptionValue("d"));
						}
