...
public class AbstractDatatype implements java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 859264437708266173L;
	...
	private HashMap<String, String> _individualRange; // 5.2.2007: HashMap used
														// to save the Types

	// parent information
	private Vector<String> _parentList;

	// rdf:type information, used for identify owl-individuals
	private Vector<String> _typeList;

	// intersection classes/types
	private Vector<String> _intersectionList;

	// sub elements
	private Vector<AbstractDatatypeElement> _properties; // type:
															// AbstractDatatypeElement

	// jena exceptions and warnings
	private Vector<String> _errorMessages;

	// owl/rdfs depending deduction information
	private Vector<String> _owlParseMessages;

	// xsd creation
	private Schema schema;

	// private AbstractDatatypeKBData ref2KBDATA; obsolete, weil
	// AbstractDatatypeKB Singleton

	/** Creates a new instance of AbstractDatatype */
	public AbstractDatatype() {
		// System.out.println("[C] AbstractDatatype_1");
		this._individualRange = new HashMap<String, String>();
		this._parentList = new Vector<String>();
		this._typeList = new Vector<String>();
		this._intersectionList = new Vector<String>();
		this._properties = new Vector<AbstractDatatypeElement>();
		this._errorMessages = new Vector<String>();
		this._owlParseMessages = new Vector<String>();
	...
	public HashMap<String, String> getIndividualRange() {
		return this._individualRange;
	}

	public Vector<String> getParentList() {
	...
	public Vector<String> getIntersectionList() {
		return this._intersectionList;
	}

	/**
	 * Get all inherited properties.
	 */
	public Vector<AbstractDatatypeElement> getProperties() {
	...
	public Vector<String> getParseMessages() {
		return this._owlParseMessages;
	}
	 ...
	 *            Inheritance level
	 */
	public Vector<AbstractDatatypeElement> getProperties(int maxDepth) {
		if (maxDepth == -1) {
			return this._properties;
		} else {
			Vector<AbstractDatatypeElement> filteredProperties = new Vector<AbstractDatatypeElement>();
			for (Iterator<AbstractDatatypeElement> it = this._properties
					.iterator(); it.hasNext();) {
	 ...
	 * if(!this._propertiesMod.isEmpty()) { System.out.println("+"); return
	 * this._propertiesMod; } Vector templist = new Vector();
	 * templist.addAll(this._properties.values()); return templist; }
	 */
	...
	public void setRdfsComment(String comment) {
		this._RDFSComment = comment;
	}

	public void setOwlVersionInfo(String versioninfo) {
		this._OWLVersionInfo = versioninfo;
	}
	...
	public void setIndividualRange(HashMap<String, String> individualRange) {
		this._individualRange = individualRange;
	}

	public void setParentList(Vector<String> parentList) {
	...
	public void setIntersectionList(Vector<String> intersectionList) {
		this._intersectionList = intersectionList;
	}

	public void setProperties(Vector<AbstractDatatypeElement> properties) {
	...
	public void setParseMessages(Vector<String> parseMessages) {
		this._owlParseMessages = parseMessages;
	}

	/*
	 * public void setPropertiesMod(Vector propertyList) {
	 * System.out.println("setPropertiesMod"); this._properties.clear();
	 * for(Iterator it=propertyList.iterator(); it.hasNext(); ) {
	 * this._properties
	 * .put(((AbstractDatatypeElement)it.next()).getName(),it.next()); } }
	 */

	public void addErrorMessage(String msg) {
		this._errorMessages.add(msg);
	}
	...
	public void addAllParseMessages(Vector<String> parseMessages) {
	 ...
	 * 
	 * @param uriID
	 *            id or about attribute of individual (oneOf)
	 * @param uriType
	 *            RDF Type of the collection element (oneOf)
	 */
	public void addIndividualRange(String uriID, String uriType) {
		this._individualRange.put(uriID, uriType);
	}

	public void addRangeList(HashMap<String, String> rangeList) {
		// this._individualRange.addAll(rangeList);
		for (Iterator<String> it = rangeList.keySet().iterator(); it.hasNext();) {
	...
	/**
	 * Cause of the fact, that individuals could be subtypes of given type we
	 * collect all types of individuals. e.g. Language, SupportedLanguage
	 * 
	 * @return vector with types names
	 */
	public Vector<String> getIndividualRangeTypes() {
		Vector<String> rangeTypes = new Vector<String>();
		for (Iterator<String> it = this._individualRange.keySet().iterator(); it
				.hasNext();) {
			String key = it.next().toString();
	...
	public void addParent(String uri) {
		this._parentList.add(uri);
	}

	public void addType(String uri) {
		this._typeList.add(uri);
	}

	public void addIntersection(String uri) {
		this._intersectionList.add(uri);
	}
	...
	public boolean containsProperty(String key) {
		for (Iterator<AbstractDatatypeElement> it = this._properties.iterator(); it
				.hasNext();) {
	...
	// return (this.getMinDepthOfProperties()<=depth &&
	// this.getMinDepthOfProperties() >=0);
	// }
	...
	public String searchInParentsForXsdType(String defaultXsdType) {
		...
		for (Iterator<String> it = this._parentList.iterator(); it.hasNext();) {
			...
			if (AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData()
				...
				if (AbstractDatatypeKB.getInstance()
						.getAbstractDatatypeKBData().get(url).hasXsdType()) {
					typeString = AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData().get(url).getXsdType();
					break;
				} else if (!AbstractDatatypeKB.getInstance()
						.getAbstractDatatypeKBData().get(url)
						.searchInRdfTypesForXsdType(defaultXsdType)
						.equals(defaultXsdType)) {
					typeString = AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData().get(url)
							.searchInRdfTypesForXsdType(defaultXsdType);
					break;
					...
					typeString = AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData().get(url)
							.searchInParentsForXsdType(defaultXsdType);
				}
			}
		}
		return typeString;
	...
	public String searchInRdfTypesForXsdType(String defaultXsdType) {
		...
		if (this._typeList.size() == 1) {
			if (this._typeList.get(0).toString()
					.equals("http://www.w3.org/2000/01/rdf-schema#Resource")) {
				return defaultXsdType;
			}
		}

		for (Iterator<String> it = this._typeList.iterator(); it.hasNext();) {
			String url = it.next().toString();
			// System.out.println("CHECK: "+url);
			if (AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData()
					.containsKey(url)) {
	 ...
	 * 
	 * @param inheritanceBehaviour
	 *            defined in AbstractDatatype. when declared as null, None
	 *            inheritance is set.
	 * @param DEFAULT_XSDTYPE
	 *            defined in AbstractDatatype. when declared as null, XSD:anyUri
	 *            is set.
	 * @return uri of primitive xsd datatype
	 */
	public String determineXsdType(String inheritanceBehaviour,
			String DEFAULT_XSDTYPE) {
		if (this.hasXsdType()) {
			...
			System.out
					.println("[i] determineXsdType TypeInheritanceBehaviour: "
							+ inheritanceBehaviour + ", DEFAULT_XSDTYPE: "
							+ DEFAULT_XSDTYPE);

			if (inheritanceBehaviour
					.equals(AbstractDatatype.InheritanceByRDFTypeOnly)) {
				return this.searchInRdfTypesForXsdType(DEFAULT_XSDTYPE);
			} else if (inheritanceBehaviour
					.equals(AbstractDatatype.InheritanceBySuperClassOnly)) {
				return this.searchInParentsForXsdType(DEFAULT_XSDTYPE);
			} else if (inheritanceBehaviour
					.equals(AbstractDatatype.InheritanceByRDFTypeFirstParentsSecond)) {
				String xsdtype = this
						.searchInRdfTypesForXsdType(DEFAULT_XSDTYPE);
				...
				String xsdtype = this
						.searchInParentsForXsdType(DEFAULT_XSDTYPE);
				if (xsdtype.equals(DEFAULT_XSDTYPE)) {
	...
	public String determineXsdType(String inheritanceBehaviour) {
		return this.determineXsdType(inheritanceBehaviour, DEFAULT_XSDTYPE);
	}

	public AbstractDatatypeElement getElement(String key) {
		for (Iterator<AbstractDatatypeElement> it = this._properties.iterator(); it
				.hasNext();) {
			AbstractDatatypeElement cur = it.next();
	...
	public void printDatatype() {
		...
		if (!this._intersectionList.isEmpty()) {
			for (Iterator<String> it = this._intersectionList.iterator(); it
					.hasNext();) {
				System.out
						.println("     Intersection: " + it.next().toString());
			}
		}
		if (!this._individualRange.isEmpty()) {
			for (Iterator<String> it = this._individualRange.keySet()
					.iterator(); it.hasNext();) {
				String key = it.next().toString();
				System.out.println("        Range Val: " + key + " ("
						+ this._individualRange.get(key) + ")");
			}
		}
		if (!this._properties.isEmpty()) {
			System.out.println("PROPERTIES (ELEMENTS)________________________");
			System.out.println(" * min depth: "
					+ this.getMinDepthOfProperties());
			for (Iterator<AbstractDatatypeElement> it = this._properties
					.iterator(); it.hasNext();) {
				it.next().printData();
				System.out.println();
	...
	public static void main(String args[]) {
		...
		AbstractDatatypeMapper.getInstance().loadAbstractDatatypeKB(
				"file:/D:/tmp/KB/KB_Student2.xml");

		// AbstractDatatype atype =
		// AbstractDatatypeKB.getInstance().data.get("file:/D:/htw_kim/thesis/OWLS-MX/owls-tc2/ontology/SUMO.owl#Vitamin");
		// AbstractDatatype atype =
		// AbstractDatatypeKB.getInstance().data.get("http://127.0.0.1/ontology/Student.owl#Number");
		// AbstractDatatype atype =
		// AbstractDatatypeKB.getInstance().data.get("http://127.0.0.1/ontology/Student.owl#MyUnitOfMeasure");
		// AbstractDatatype atype =
		// AbstractDatatypeKB.getInstance().data.get("http://127.0.0.1/ontology/Student.owl#NonnegativeInteger");
		AbstractDatatype atype = AbstractDatatypeKB.getInstance().data
				.get("http://127.0.0.1/ontology/Student.owl#HTWMasterStudent");
		// atype.printDatatype();

		System.out.println("Determinate XSD type-1: "
				+ atype.determineXsdType(AbstractDatatype.InheritanceByNone));
		System.out
				.println("Determinate XSD type-2: "
						+ atype.determineXsdType(AbstractDatatype.InheritanceByRDFTypeFirstParentsSecond));
		System.out
				.println("Determinate XSD type-3: "
						+ atype.determineXsdType(AbstractDatatype.InheritanceByParentsFirstRDFTypeSecond));
		...
		try {
			XsdSchemaGenerator gen = new XsdSchemaGenerator("WSDLTYPE", false,
					1, AbstractDatatype.InheritanceByRDFTypeFirstParentsSecond,
