 ...
 * @author Oliver
 */
public class AbstractDatatypeKB implements java.io.Serializable { // extends
																	// java.util.Observable

	/**
	 * 
	 */
	private static final long serialVersionUID = 927845421214133701L;
	...
	public void addDatatype(AbstractDatatype atype) {
		...
		// this.setChanged(); // notify nachdem alle neuen Typen eingepflegt
		// worden sind.
	}

	public void printFullStatus() {
		System.out.println("Registered Datatypes: "
				+ this.data.getRegisteredDatatypes().size());
		System.out.println("ErroneousDatatypes  : "
				+ this.data.getErroneousDatatypes().size());
		System.out.println("MetaDatatypes       : "
				+ this.data.getMetaDatatypes().size());
		System.out.println("Modification Time   : "
				+ this.data.getModificationTime());
		for (Iterator<AbstractDatatype> it = this.data.getRegisteredDatatypes()
				.values().iterator(); it.hasNext();) {
			it.next().printDatatype();
		}
		System.out.println("META DATATYPES___________________________________");
		for (Iterator<AbstractDatatype> it = this.data.getMetaDatatypes()
				.values().iterator(); it.hasNext();) {
			it.next().printDatatype();
		}
	...
	public void printErroneousDatatypes() {
		Vector<String> list = new Vector<String>(this.data
				.getErroneousDatatypes().keySet());
		for (Iterator<String> it = list.iterator(); it.hasNext();) {
			String name = it.next().toString();
			System.out.println(name + ": "
					+ this.data.getErroneousDatatypes().get(name));
		}
	}

	public void printMetaDatatypes() {
		Vector<String> list = new Vector<String>(this.data.getMetaDatatypes()
				.keySet());
		System.out.println("MetaDatatypes Count: " + list.size());
		for (Iterator<String> it = list.iterator(); it.hasNext();) {
			String name = it.next().toString();
			System.out.println("METADATA: " + name + ": "
					+ this.data.getMetaDatatypes().get(name).getLocalName());
		}
	}
	...
	// XML SCHEMA
	//
	public void toXSD(String uri, boolean useHierarchyPattern, int deepness,
			String xsdInheritance, String xsdDefaultType) throws Exception {
		if (this.data.getRegisteredDatatypes().containsKey(uri)) {
			System.out.println("[toXSD] ENTHALTEN: " + uri);
			AbstractDatatype curtype = (AbstractDatatype) this.data
					.getRegisteredDatatypes().get(uri);

			// new SchemaGenerator
			gen = new XsdSchemaGenerator("WSDLTYPE", useHierarchyPattern,
					deepness, xsdInheritance, xsdDefaultType);
			gen.toXSD(curtype);
		} else {
	...
	public void toXSD(String uri, XsdSchemaGenerator xsdgen, OutputStream out)
			throws Exception {
		if (this.data.getRegisteredDatatypes().containsKey(uri)) {
			System.out.println("[toXSD] ENTHALTEN: " + uri);
			AbstractDatatype curtype = (AbstractDatatype) this.data
	...
	public void toXSD(String uri, boolean useHierarchyPattern, int deepness,
			String xsdInheritance, String xsdDefaultType, OutputStream out)
			throws Exception {
		this.toXSD(uri, useHierarchyPattern, deepness, xsdInheritance,
				xsdDefaultType);
		this.printXSD(out);
	}
	...
	public Schema getXmlSchemaElement(String url, boolean useHierarchyPattern,
			int deepness, String xsdInheritance) throws Exception {
		Schema curSchema = null;
		if (this.data.getRegisteredDatatypes().containsKey(url)) {
			AbstractDatatype curtype = (AbstractDatatype) this.data
					.getRegisteredDatatypes().get(url);

			XsdSchemaGenerator gen = new XsdSchemaGenerator("WSDLTYPE",
					useHierarchyPattern, deepness, xsdInheritance);
			gen.toXSD(curtype);
	...
	public void marshallAsXML(OutputStream out, boolean prettyprint) {
		AbstractDatatypeMapper.getInstance().mapAbstractDatatypeKB(data, out,
				prettyprint);
	}
}
