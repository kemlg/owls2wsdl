...
public class RegisteredDatatypeElementTableModel extends AbstractTableModel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3690227652478427306L;
	AbstractDatatypeElement _elem = null;
	Vector<AbstractDatatypeRestrictionElement> _collectedRestrictionValues;
	...
	public RegisteredDatatypeElementTableModel() {
		this._collectedRestrictionValues = new Vector<AbstractDatatypeRestrictionElement>();
	}
	...
	public void updateModel(AbstractDatatypeElement elem) {
		...
		if (!elem.getRestrictions().isEmpty()) {
			...
			System.out
					.println("[i] RegisteredDatatypeElementTableModel: update with elem.getType: "
							+ elem.getType());
			if (AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData()
					.containsKey(elem.getType())) {
				if (elem.getOwlSource().equals("OBJECT")
						|| elem.getOwlSource().equals("RESTRICTION-ON-PROJECT")) {
					Set<String> individualRangeSet = AbstractDatatypeKB
							.getInstance().data.get(elem.getType())
							.getIndividualRange().keySet();
					if (individualRangeSet.isEmpty()) {
						System.out.println("[i] individualRangeSet is empty.");
					} else {
						for (Iterator<String> it = individualRangeSet
								.iterator(); it.hasNext();) {
							String rangeURI = it.next().toString();
							String rangeType = AbstractDatatypeKB.getInstance().data
									.get(elem.getType()).getIndividualRange()
									.get(rangeURI).toString();
							this._collectedRestrictionValues
									.add(new AbstractDatatypeRestrictionElement(
											rangeType, rangeURI, 0, null));
						}
					}
				}
			} else {
				System.out
						.println("[e] "
								+ elem.getType()
								+ " not in KB. No more inforamtion about element range available.");
			}
		}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		if (columnIndex == 0)
			return ((AbstractDatatypeRestrictionElement) this._collectedRestrictionValues
					.get(rowIndex)).getRestrictionType();
		else
			return ((AbstractDatatypeRestrictionElement) this._collectedRestrictionValues
					.get(rowIndex)).getRestrictionValue();
	}
	...
	//
	// // AbstractDatatypeElement testElem =
	// (AbstractDatatypeElement)curtype.getProperties().get("http://www.w3.org/TR/2003/PR-owl-guide-20031209/wine#hasColor");
	// // testElem.printData();
	// //
	// // RegisteredDatatypeElementTableModel model = new
	// RegisteredDatatypeElementTableModel();
