...
public class StatusTable extends JTable implements java.util.Observer {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7502139221488588510L;
	...
	private static Object[][] data = {
			...
			{ "Services 1.2", new Integer(0), new Integer(0) },
			{ "Metatypes (owl:class)", new Integer(0), new Integer(0) },
			{ "Metatypes (anonymous)", new Integer(0), new Integer(0) } };
	...
	public StatusTable() {
		...
		/*
		 * not initialized this.getModel().setValueAt(new
		 * Integer(AbstractDatatypeKB
		 * .getInstance().data.getRegisteredDatatypes().size()), 2, 2);
		 * this.getModel().setValueAt(new
		 * Integer(RuntimeModel.getInstance().getStatus("10SERVICES")), 0, 2);
		 * this.getModel().setValueAt(new
		 * Integer(RuntimeModel.getInstance().getStatus("11SERVICES")), 1, 2);
		 */
	}
	...
	public void update(Observable observable, Object object) {
		if (RuntimeModel.getInstance().getStatus("RUNTIME")
			...
			int service11Count = RuntimeModel.getInstance().getProject()
					.getAbstractServiceCollection().getCount11();
			int service12Count = RuntimeModel.getInstance().getProject()
					.getAbstractServiceCollection().getCount12();

			this.getModel().setValueAt(Integer.valueOf(service10Count), 0, 1);
			this.getModel().setValueAt(Integer.valueOf(service11Count), 1, 1);
			this.getModel().setValueAt(Integer.valueOf(service12Count), 2, 1);
			...
			this.getModel().setValueAt(
					new Integer(AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData()
							.getRegisteredDatatypes().size()), 3, 1);
			this.getModel().setValueAt(
					new Integer(AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData()
							.getErroneousDatatypes().size()), 3, 2);
			this.getModel().setValueAt(
					new Integer(AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData().getMetaDatatypes()
							.size()), 4, 1);
