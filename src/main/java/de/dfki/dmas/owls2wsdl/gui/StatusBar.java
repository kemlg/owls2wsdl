...
public class StatusBar extends JPanel implements Observer {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 330103688490602680L;
	...
	private final Font globalStatusBarFontBold = new Font("Dialog", Font.BOLD,
			11);
	...
	public StatusBar() {
		...
		this.setBorder(new CompoundBorder(new SoftBevelBorder(
				SoftBevelBorder.LOWERED), new EmptyBorder(0, 2, 0, 2)));
	...
	public void update(java.util.Observable o, Object arg) {
		...
		if (RuntimeModel.getInstance().getStatus("RUNTIME")
				.equals(RuntimeModel.PROPERTIES_CHANGED)) {
			int depth = RuntimeModel.getInstance().getProject()
					.getElementDepth();
			this.depthStatus.setText(Integer.toString(depth));
			if (RuntimeModel.getInstance().getSelectedDatatype() != null) {
				this.xsdStatus.setText(RuntimeModel.getInstance()
						.getSelectedDatatype().getResultingXsdType(depth));
			}
		} else if (RuntimeModel.getInstance().getStatus("RUNTIME")
				.equals(RuntimeModel.SINGLE_SERVICE_SELECTED)) {
			// arg muss null sein, damit beim Umschalten Service / Datatype kein
			// null Wert gelesen wird
			if (((RuntimeModel) o).getSelectedService() != null) {
				String servicename = ((RuntimeModel) o).getSelectedService()
						.getName();
				this.selectedServiceLabel.setText(servicename);

				if (((RuntimeModel) o).getSelectedService().istranslatable()) {
					this.selectedServiceInformationLabel
				...
				.equals(RuntimeModel.SINGLE_DATATYPE_SELECTED)) {
			if (((RuntimeModel) o).getSelectedDatatype() != null) {
				this.selectedDatatypeLabel.setText(((RuntimeModel) o)
						.getSelectedDatatype().getLocalName());
				int depth = RuntimeModel.getInstance().getProject()
						.getElementDepth();
				this.xsdStatus.setText(RuntimeModel.getInstance()
						.getSelectedDatatype().getResultingXsdType(depth));
			...
			if (RuntimeModel.getInstance().getSelectedDatatype() != null) {
				this.selectedDatatypeLabel.setText(RuntimeModel.getInstance()
						.getSelectedDatatype().getLocalName());
			} else {
			...
			if (RuntimeModel.getInstance().getSelectedService() != null) {
				String servicename = RuntimeModel.getInstance()
						.getSelectedService().getName();
				this.selectedServiceLabel.setText(servicename);
			} else {
