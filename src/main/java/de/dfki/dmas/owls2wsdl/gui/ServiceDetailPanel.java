...
import de.dfki.dmas.owls2wsdl.gui.models.ServiceParameterTableColumnModel;

/**
 * 
 * @author Oliver Fourman
 */
public class ServiceDetailPanel extends JComponent implements
		ListSelectionListener, Observer {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1381596961838234874L;
	...
	private JList<String> serviceNamespaceList; // JTP
	private JList<String> serviceImportedFilesList; // JTP
	...
	private void initComponents() {
		...
		serviceParameterTable.setPreferredScrollableViewportSize(new Dimension(
				600, 80));
		serviceParameterTable.updateUI();
		serviceParameterTable.getSelectionModel().setSelectionMode(
				ListSelectionModel.SINGLE_SELECTION);
		serviceParameterTable.getSelectionModel()
		...
		serviceParameterTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					// int column = serviceParameterTable.columnAtPoint(p); //
					// This is the view column!
					// System.out.println("DOUBLE CLICK: "+row+", "+column);
					System.out.println("[TABLE POS   : "
							+ ((JTable) e.getSource()).getSelectedRow());
					AbstractServiceParameter param = serviceParameterModel
							.getAbstractServiceParameter(((JTable) e
									.getSource()).getSelectedRow());
					if (AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData()
							.containsKey(param.getUri())) {
						System.out
								.println("[ServiceDetailPanel] CHANGE selected AbstractDatatype");
						AbstractDatatype atype = AbstractDatatypeKB
						...
						RuntimeModel.getInstance().setRuntimeAndNotify(
								RuntimeModel.PARAMETER_SELECTED);
					} else {
						JOptionPane.showMessageDialog(null,
								"No type information for parameter in KB.",
								"Parameter Failure", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		});
		...
		serviceNamespaceList = new JList<String>(new DefaultListModel<String>());
		serviceNamespaceList.setVisibleRowCount(5);
		serviceNamespaceList.setEnabled(false);

		serviceImportedFilesList = new JList<String>(
	...
	public void buildPanel() {
		...
		FormLayout layout = new FormLayout(
				"right:pref, 10px, left:0:grow, 10px, right:pref, 10px, fill:pref:grow",
		...
		this.add(
				DefaultComponentFactory.getInstance().createSeparator(
						ResourceManager.getString("service.sep.info")),
				cc.xyw(1, 1, 7));
		...
		this.add(
				DefaultComponentFactory.getInstance().createSeparator(
						ResourceManager.getString("service.sep.interface")),
				cc.xyw(1, 11, 7));
	...
	// FormLayout("pref, 10px, pref:grow"));
	// CellConstraints cc = new CellConstraints();
	//
	...
	public void update(Observable observable, Object object) {
		...
		System.out.println("[u] update ServiceDetailPanel, Runtime: "
				+ RuntimeModel.getInstance().getStatus("RUNTIME"));

		if (RuntimeModel.getInstance().getStatus("RUNTIME")
				.equals(RuntimeModel.SERVICE_MODEL_CHANGED)
				|| RuntimeModel.getInstance().getStatus("RUNTIME")
						.equals(RuntimeModel.SINGLE_SERVICE_SELECTED)) {
	...
	public void clearPanel() {
		...
		for (int i = 0; i < sdpComponents.length; i++) {
			if (sdpComponents[i].getClass().getName()
					.equals("javax.swing.JTextField")
					|| sdpComponents[i].getClass().getName()
							.equals("javax.swing.JTextPane")) {
		...
		((ServiceParameterTableModel) serviceParameterTable.getModel())
				.removeAllParameter();
		...
		((DefaultListModel<String>) serviceNamespaceList.getModel())
				.removeAllElements();
		serviceNamespaceList.revalidate();

		((DefaultListModel<String>) serviceImportedFilesList.getModel())
				.removeAllElements();
	...
	public void updateServiceDetailPanel() {
		...
		final DefaultListModel<String> model_1 = new DefaultListModel<String>();
		final DefaultListModel<String> model_2 = new DefaultListModel<String>();

		for (Iterator<String> it = aservice.getNamespaceEntries().values()
				.iterator(); it.hasNext();) {
		...
		for (Iterator<String> it = aservice.getImportedOWLFiles().iterator(); it
				.hasNext();) {
