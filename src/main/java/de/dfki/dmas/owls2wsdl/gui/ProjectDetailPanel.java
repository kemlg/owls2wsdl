...
import de.dfki.dmas.owls2wsdl.gui.models.ProjectLoadedOntListModel;
import de.dfki.dmas.owls2wsdl.gui.models.ProjectServiceDependencyTypesTableColumnModel;
import de.dfki.dmas.owls2wsdl.gui.models.ProjectServiceDependencyTypesTableModel;
...
public class ProjectDetailPanel extends JPanel implements Observer {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4300348645345345019L;
	private ProjectLoadedOntListModel ontlistModel;
	private JList<String> parsedOntologiesList;
	...
	private void initComponents() {
		...
		parsedOntologiesList = new JList<String>(ontlistModel);
		parsedOntologiesList.setVisibleRowCount(7);

		changesList = new JList<String>();
		...
		serviceDependencyTypesTable = new JTable(
				serviceDependencyTypesTableModel, cm);
		serviceDependencyTypesTable
		...
		projectTab = new JTabbedPane(JTabbedPane.TOP,
		...
		removeSelectedTypeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String key = serviceDependencyTypesTable.getModel()
						.getValueAt(idx, 1).toString();
	...
	private void buildPanel(boolean debugmode) {
		...
		projectTab.add("Service Dependencies",
				buildServiceDependenciesInfoTab(debugmode));
	...
	private JScrollPane buildOntInfoTab(boolean debugmode) {
		...
		FormLayout layout = new FormLayout("fill:pref:grow",
				"pref, 3dlu, pref, 5dlu, " + "pref, 3dlu, pref");
	...
	private JScrollPane buildServiceDependenciesInfoTab(boolean debugmode) {
		...
		CellConstraints cc = new CellConstraints();
		builder.addSeparator("Service dependencies and missing types",
				cc.xyw(1, 1, 9));
		builder.add(new JScrollPane(serviceDependencyTypesTable),
				cc.xyw(1, 3, 9));
	...
	public void updatePanel() {
		...
		datatypesTotalCountField.setText(String.valueOf(AbstractDatatypeKB
				.getInstance().getAbstractDatatypeKBData()
				.getRegisteredDatatypes().size()));
		serviceDependencyTypesCountField
				.setText(String.valueOf(RuntimeModel.getInstance().getProject()
						.getServiceDependencyTypes().size()));
		serviceMissingTypesCountField.setText(String.valueOf(RuntimeModel
				.getInstance().getProject().getServiceMissingTypes().size()));
	...
	public void update(Observable o, Object arg) {
		...
		if (RuntimeModel.getInstance().getStatus("RUNTIME")
			...
			System.out.println("[i] Ont Count: "
					+ AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData().getOntologyURIs()
							.size());
			updatePanel();
			this.updateUI();
		}
	...
	public static void main(String[] args) {
		File f = new File(
				"D:\\tmp\\OWLS2WSDL-project-car_price_service\\car_price_service.xml");
