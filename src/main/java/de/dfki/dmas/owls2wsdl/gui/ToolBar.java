...
public class ToolBar extends JToolBar implements Observer {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3098439396313062239L;
	final URL resDocumentNew = ToolBar.class
			.getResource("/images/22x22/actions/document-new.png");
	final URL resDocumentOpen = ToolBar.class
			.getResource("/images/22x22/actions/document-open.png");
	final URL resDocumentSave = ToolBar.class
			.getResource("/images/22x22/actions/document-save.png");
	final URL resTranslate = ToolBar.class
			.getResource("/images/22x22/actions/format-indent-more.png");
	final URL resMindswapIcon = ToolBar.class
			.getResource("/images/22x22/mindswap22x22.png");
	...
	private JButton newProjectButton, loadProjectButton, saveProjectButton,
			generateXSDButton, generateWSDLButton,
			showWSDL2OWLSConverterButton, generateOWLSButton;
	...
	public ToolBar(GUIActionListener actionListener) {
		...
		this.newProjectButton = new JButton(
				ResourceManager.getString("project.new"), iconNew);
		...
		this.loadProjectButton = new JButton(
				ResourceManager.getString("project.open"), iconOpen);
		...
		this.saveProjectButton = new JButton(
				ResourceManager.getString("project.save"), iconSave);
		...
		generateWSDLButton.setToolTipText(ResourceManager
				.getString("toolbar.owls2wsdl.tooltip"));
		...
		generateOWLSButton.setToolTipText(ResourceManager
				.getString("toolbar.owls.tooltip"));
		...
		generateXSDButton.setToolTipText(ResourceManager
				.getString("toolbar.owl2xsd.tooltip"));
		...
		showWSDL2OWLSConverterButton = new JButton("WSDL2OWL-S Converter",
				iconMindswap);
		showWSDL2OWLSConverterButton
				.setToolTipText("Generate OWL-S by reengineering WSDL (OWL-S GUI)");
		showWSDL2OWLSConverterButton.setEnabled(true);
		showWSDL2OWLSConverterButton.addActionListener(this.actionListener);
		showWSDL2OWLSConverterButton
				.setActionCommand(GUIActionListener.SHOW_WSDL2OWLS_CONVERTER);
	...
	private void deactivateAllButtons() {
		...
		for (int i = 0; i < componentList.length; i++) {
			if (componentList[i].getClass().getName()
	...
	public void update(java.util.Observable o, Object arg) {
		...
		if (RuntimeModel.getInstance().getProject() == null) {
			...
			if (RuntimeModel.getInstance().getStatus("RUNTIME")
				...
				if (RuntimeModel.getInstance().getSelectedService()
						.istranslatable()) {
					this.generateWSDLButton.setEnabled(true);
					this.showWSDL2OWLSConverterButton.setEnabled(true); // WSDL2OWL-S
																		// Converter
					this.generateOWLSButton.setEnabled(true);
				} else {
					this.generateWSDLButton.setEnabled(false);
					this.showWSDL2OWLSConverterButton.setEnabled(false); // WSDL2OWL-S
																			// Converter
