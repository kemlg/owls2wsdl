...
public class ConfigurationFrame extends JDialog implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2875433490201746192L;
	...
	private JComboBox<String> xsdTypeInheritanceBehaviour;
	...
	private JComboBox<String> projectElementDepthField;
	private JComboBox<String> projectDefaultXsdTypeField;
	...
	public ConfigurationFrame(OWLS2WSDLGui frame) {
		...
		this.contentPane.setBorder(Borders.createEmptyBorder(Sizes.pixel(10),
				Sizes.pixel(10), Sizes.pixel(10), Sizes.pixel(10)));
		...
		this.persistentDatatypeCheckBox = new JCheckBox(
				"load referenced datatypes only (experimental)");
		...
		this.buildRelativePathCheckBox = new JCheckBox(
				"extract in relative paths");
		...
		xsdTypeInheritanceBehaviour
				.addItem(AbstractDatatype.InheritanceBySuperClassOnly);
		xsdTypeInheritanceBehaviour
				.addItem(AbstractDatatype.InheritanceByRDFTypeOnly);
		xsdTypeInheritanceBehaviour
				.addItem(AbstractDatatype.InheritanceByRDFTypeFirstParentsSecond);
		xsdTypeInheritanceBehaviour
				.addItem(AbstractDatatype.InheritanceByParentsFirstRDFTypeSecond);

		String[] primitiveSchemaTypes = {
		...
		hierarchyButton = new JRadioButton(
				"use hierarchy pattern (subclassing)");
		...
		addMetaTypesCheckBox = new JCheckBox(
				"add meta types to express OWL anonymous language terms");
		addAnnotationCheckBox = new JCheckBox(
				"add XML Schema annotations (general and OWL information)");
		...
		this.autoNamespaceBaseSelectButton = new JRadioButton(
				"auto (using parsed service data)");
		this.autoNamespaceBaseSelectButton.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					System.out
							.println("autoNamespaceBaseSelectButton selected");
					wsdlNamespaceField.setEditable(false);
					wsdlNamespaceField.setEnabled(false);
				}
			}
		});
		this.newNamespaceBaseSelectButton = new JRadioButton(
				"set new tns: <base_path><service>");
		...
		this.autoOWLSBaseSelectButton = new JRadioButton(
				"auto (using parsed service data)");
		...
		this.newOWLSBaseSelectButton = new JRadioButton(
				"set new owl-s base: <base_path><OWLS_DOC>");
		...
		this.autoWsdldocPathSelectButton = new JRadioButton(
				"auto (use WSDL Builder setting above)");
		...
		this.setWsdlDocPathSelectButton = new JRadioButton(
				"set new address path: <base_path><WSDL_DOC>");
	...
	private JComponent buildConfigSubPanel(boolean debugmode) {
		...
		configTabbedPane.add(
				"XSD Generation",
				new JScrollPane(this
						.buildSubPanel4XsdGenerationConfigs(debugmode)));
		configTabbedPane.add("WSDL- / OWL-S Builder",
				new JScrollPane(this.buildExportSettingsTab(debugmode)));
		configTabbedPane.add(
				"General Settings",
				new JScrollPane(this
						.buildSubPanel4ApplicationConfigs(debugmode)));
		return configTabbedPane;
	}

	private JPanel buildSubPanel4ApplicationConfigs(boolean debugmode) {
		FormLayout layout = new FormLayout(
				"10dlu, right:pref, 10px, fill:pref:grow, 5px, pref");
		...
		builder.append("Application path", this.applPathField,
				this.applPathBrowseButton);
		builder.append("Export directory", this.exportDirectoryField,
				this.exportDirectoryBrowseButton);
		builder.append("Log file", this.logPathField, 3);
		builder.append("Persistent datatype directory",
				this.persistentDatatypeDirectoryField, 3);
		builder.append("Project directory",
				this.persistentProjectDirectoryField, 3);
	...
	private JPanel buildSubPanel4XsdGenerationConfigs(boolean debugmode) {
		FormLayout layout = new FormLayout(
				"10dlu, right:pref, 10px, fill:pref:grow");
		DefaultFormBuilder builder = null;
		if (debugmode)
			builder = new DefaultFormBuilder(layout, new FormDebugPanel());
		else
		...
		builder.append("Default inheritance depth",
				this.defaultElementDepthField, 1);
		...
		builder.append("Enhance WSDL Matchmaking", this.addAnnotationCheckBox,
				1);

		builder.appendSeparator("Project Properties (saved in project)");
		builder.append("Use this depth to add elements",
				this.projectElementDepthField, 1);
		builder.append("Inherit XSD Type from",
				this.xsdTypeInheritanceBehaviour, 1);
	...
	private JPanel buildExportSettingsTab(boolean debugmode) {
		...
		builder.append("Change Namespace (tns)",
				this.autoNamespaceBaseSelectButton, 1);
		...
		builder.append("Set grounding path", this.autoWsdldocPathSelectButton,
				1);
				...
				if (RuntimeModel.getInstance().getProject() != null) {
					RuntimeModel.getInstance()
							.setStatusAndNotify("RUNTIME",
									RuntimeModel.PROPERTIES_CHANGED,
									"saved properties");
				}
				// setVisible(false);
				dispose();
	...
	public void initContentsFromConfigurationFile() {
		this.applPathField.setText(OWLS2WSDLSettings.getInstance().getProperty(
				"APPL_PATH"));
		this.logPathField.setText(OWLS2WSDLSettings.getInstance().getProperty(
				"logfile"));

		this.persistentProjectDirectoryField.setText(OWLS2WSDLSettings
				.getInstance().getProperty("PROJECT_DIR"));
		this.persistentDatatypeDirectoryField.setText(OWLS2WSDLSettings
				.getInstance().getProperty("PERSISTENT_DATATYPE_DIR"));

		if (OWLS2WSDLSettings.getInstance().getProperty("JCONSOLE")
				.equals("yes")) {
		...
		if (OWLS2WSDLSettings.getInstance()
				.getProperty("RELEVANT_DATATYPES_ONLY").equals("yes")) {
			this.persistentDatatypeCheckBox.setSelected(true);
		} else {
			this.persistentDatatypeCheckBox.setSelected(false);
		}

		this.defaultElementDepthField.setText(OWLS2WSDLSettings.getInstance()
				.getProperty("depth"));

		if (OWLS2WSDLSettings.getInstance().getProperty("xsdgen")
				.equals("short")) {
			this.shortButton.setSelected(true);
		} else if (OWLS2WSDLSettings.getInstance().getProperty("xsdgen")
				.equals("hierarchy")) {
			this.hierarchyButton.setSelected(true);
		}

		if (OWLS2WSDLSettings.getInstance().getProperty("owlinfo")
				.equals("yes")) {
		...
		if (OWLS2WSDLSettings.getInstance().getProperty("annotations")
				.equals("yes")) {
		...
		this.exportDirectoryField.setText(OWLS2WSDLSettings.getInstance()
				.getProperty("EXPORT_PATH"));

		if (OWLS2WSDLSettings.getInstance().getProperty("EXPORT_MODE")
				.equals("relative")) {
		...
		} else if (OWLS2WSDLSettings.getInstance().getProperty("lang")
				.equals("en")) {
		...
		if (OWLS2WSDLSettings.getInstance().getProperty("laf")
				.equals("windows")) {
			this.windowsLfButton.setSelected(true);
		} else if (OWLS2WSDLSettings.getInstance().getProperty("laf")
				.equals("motif")) {
		...
		this.projectElementDepthField.setSelectedItem(OWLS2WSDLSettings
				.getInstance().getProperty("default_element_depth"));

		this.wsdlNamespaceField.setText(OWLS2WSDLSettings.getInstance()
				.getProperty("TNS_BASEPATH"));
		...
		this.owlsBaseField.setText(OWLS2WSDLSettings.getInstance().getProperty(
				"BASE"));
		if (OWLS2WSDLSettings.getInstance().getProperty("CHANGE_BASE")
				.equals("yes")) {
		...
		this.wsdldocPathField.setText(OWLS2WSDLSettings.getInstance()
				.getProperty("WSDLPATH"));
		if (OWLS2WSDLSettings.getInstance().getProperty("CHANGE_WSDLPATH")
				.equals("yes")) {
	...
	public void updateContentsFromProjectData() {
		...
		if (RuntimeModel.getInstance().getProject() == null) {
			...
			this.xsdTypeInheritanceBehaviour.setSelectedItem(RuntimeModel
					.getInstance().getProject().getTypeInheritanceBehaviour());
			this.xsdTypeInheritanceBehaviour.setEnabled(true);
			this.projectElementDepthField.setSelectedItem(Integer
					.toString(RuntimeModel.getInstance().getProject()
							.getElementDepth()));
			this.projectElementDepthField.setEnabled(true);
			this.projectDefaultXsdTypeField.setSelectedItem(RuntimeModel
					.getInstance().getProject().getDefaultXsdType());
			this.projectDefaultXsdTypeField.setEnabled(true);
		}
	}

	public void saveContentsInConfigurationFile() {
		OWLS2WSDLSettings.getInstance().setProperty("APPL_PATH",
				this.applPathField.getText());
		OWLS2WSDLSettings.getInstance().setProperty("logfile",
				this.logPathField.getText());
		OWLS2WSDLSettings.getInstance().setProperty("PROJECT_DIR",
				this.persistentProjectDirectoryField.getText());
		OWLS2WSDLSettings.getInstance().setProperty("PERSISTENT_DATATYPE_DIR",
				this.persistentDatatypeDirectoryField.getText());
		...
		if (this.persistentDatatypeCheckBox.isSelected()) {
			OWLS2WSDLSettings.getInstance().setProperty(
					"RELEVANT_DATATYPES_ONLY", "yes");
		} else {
			OWLS2WSDLSettings.getInstance().setProperty(
					"RELEVANT_DATATYPES_ONLY", "no");
		}

		OWLS2WSDLSettings.getInstance().setProperty("depth",
				this.defaultElementDepthField.getText());
		...
		OWLS2WSDLSettings.getInstance().setProperty("EXPORT_PATH",
				this.exportDirectoryField.getText());

		if (this.buildRelativePathCheckBox.isSelected()) {
			OWLS2WSDLSettings.getInstance().setProperty("EXPORT_MODE",
					"relative");
		} else {
			OWLS2WSDLSettings.getInstance().setProperty("EXPORT_MODE",
					"absolute");
		...
		OWLS2WSDLSettings.getInstance().setProperty("TNS_BASEPATH",
				this.wsdlNamespaceField.getText());
		...
		OWLS2WSDLSettings.getInstance().setProperty("BASE",
				this.owlsBaseField.getText());
		...
		OWLS2WSDLSettings.getInstance().setProperty("WSDLPATH",
				this.wsdldocPathField.getText());
		if (this.autoWsdldocPathSelectButton.isSelected()) {
			OWLS2WSDLSettings.getInstance()
					.setProperty("CHANGE_WSDLPATH", "no");
		} else {
			OWLS2WSDLSettings.getInstance().setProperty("CHANGE_WSDLPATH",
					"yes");
		}
		...
		if (this.deLangButton.isSelected()) {
			if (!OWLS2WSDLSettings.getInstance().getProperty("lang")
					.equals("de")) {
				JOptionPane.showMessageDialog(null, ResourceManager
						.getString("frame.config.lang.changemsg"),
						"Localization Info", JOptionPane.INFORMATION_MESSAGE);
			}
	...
	public void updateProjectData() {
		if (RuntimeModel.getInstance().getProject() != null) {
			RuntimeModel
					...
					.getProject()
					.setElementDepth(
							Integer.parseInt(this.projectElementDepthField
									.getSelectedItem().toString()));
			RuntimeModel
					.getInstance()
					.getProject()
					.setTypeInheritanceBehaviour(
							this.xsdTypeInheritanceBehaviour.getSelectedItem()
									.toString());
			RuntimeModel
					.getInstance()
					.getProject()
					.setDefaultXsdType(
							this.projectDefaultXsdTypeField.getSelectedItem()
									.toString());
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		try {
			if (e.getActionCommand().equals(SET_APPL_PATH)) {
				String applPath = System.getProperty("user.home")
						+ File.separator + "owls2wsdl";
				if (OWLS2WSDLSettings.getInstance().containsKey("APPL_PATH")) {
					applPath = OWLS2WSDLSettings.getInstance().getProperty(
							"APPL_PATH");
				}
				applPath = this.doBrowse(applPath);
				this.applPathField.setText(applPath);
				this.logPathField.setText(applPath + File.separator
						+ "owls2wsdl.log");
			} else if (e.getActionCommand().equals(SET_EXPORT_DIRECTORY)) {
				String exportPath = System.getProperty("user.home")
						+ File.separator + "owls2wsdl";
				if (OWLS2WSDLSettings.getInstance().containsKey("EXPORT_PATH")) {
					exportPath = OWLS2WSDLSettings.getInstance().getProperty(
							"EXPORT_PATH");
				}
	...
	private void switchLAF(String lafName) {
		try {
			...
			System.out.println("[e] InstantiationException: "
					+ iae.getMessage());
		} catch (InstantiationException ie) {
			System.out
					.println("[e] InstantiationException: " + ie.getMessage());
		} catch (ClassNotFoundException cnfe) {
			System.out.println("[e] ClassNotFoundException: "
					+ cnfe.getMessage());
		} catch (UnsupportedLookAndFeelException uslaf) {
			System.out.println("[e] UnsupportedLookAndFeelException: "
					+ uslaf.getMessage());
		}
	}
