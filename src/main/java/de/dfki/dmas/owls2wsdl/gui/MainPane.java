...
final class MainPane implements ChangeListener, Observer {
	...
	final URL owls2wsdlLogoURL = MainPane.class
	...
	public MainPane() {
		...
		serviceList.getSelectionModel().setSelectionMode(
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					if (serviceList.getSelectedValuesList().size() > 1) {
						...
						RuntimeModel.getInstance().setStatusAndNotify(
								"RUNTIME",
						...
						RuntimeModel.getInstance().setSelectedService(
								((ServiceListModel) serviceList.getModel())
										.getElementAt(serviceList
												.getSelectedIndex()));
						RuntimeModel.getInstance().setStatusAndNotify(
								"RUNTIME",
								RuntimeModel.SINGLE_SERVICE_SELECTED,
								"single service");
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					if (datatypeList.isSelectionEmpty()) {
						System.out
								.println("[i] MainPane.datatypeList: no selection, nothing triggerd.");
					} else {
						RuntimeModel.getInstance().setSelectedDatatype(
								((AbstractDatatypeListModel) datatypeList
										.getModel()).getElementAt(datatypeList
										.getSelectedIndex()));
						RuntimeModel.getInstance().setStatusAndNotify(
								"RUNTIME",
								RuntimeModel.SINGLE_DATATYPE_SELECTED,
								"single type sel.");
						rdtdp.updatePanel(((AbstractDatatypeListModel) datatypeList
								.getModel()).getElementAt(datatypeList
								.getSelectedIndex()));
					}
				}
		...
		datatypeList.getSelectionModel().setSelectionMode(
	...
	public void stateChanged(ChangeEvent event) {
		this.sif_overview.setTitle(tabbedPaneLeft.getTitleAt(tabbedPaneLeft
				.getSelectedIndex()));
	...
	public void showWSDL2OWLSConverter(boolean flag) {
		if (flag) {
			((JSplitPane) horizontalSplitComponent)
					.setRightComponent(converterFrame);
		} else {
			((JSplitPane) horizontalSplitComponent)
					.setRightComponent(mainRightComponentPanel);
		}
	...
	public Vector<AbstractService> getSelectedServicesList() {
		...
		for (int i = 0; i < this.serviceList.getSelectedIndices().length; i++) {
			...
			selectedAbstractServices.add(((ServiceListModel) serviceList
					.getModel()).getElementAt(selectionIndex));
		}
		return selectedAbstractServices;
	}

	public void update(java.util.Observable o, Object arg) {
		if (RuntimeModel.getInstance().getRuntime()
				.equals(RuntimeModel.PARAMETER_SELECTED)) {
			AbstractDatatype atype = RuntimeModel.getInstance()
					.getSelectedDatatype();
			System.out
					.println("[u] MainPane: new datatype (parameter) selected: "
							+ atype.getUrl());
		...
		} else if (RuntimeModel.getInstance().getRuntime()
				.equals(RuntimeModel.DATATYPE_MODEL_CHANGED)) {
			this.datatypeListModel.updateModel();
			this.datatypeList.revalidate();
			this.datatypeList.updateUI();
		} else if (RuntimeModel.getInstance().getRuntime()
				.equals(RuntimeModel.PROJECT_LOADED)) {
	...
	private JComponent buildHorizontalSplit() {
		return Factory.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				buildMainLeftPanel(), mainRightComponentPanel, 0.2f);
	}
	...
	private JComponent buildMainLeftPanel() {
		...
		tabbedPaneLeft.putClientProperty(Options.EMBEDDED_TABS_KEY,
				Boolean.TRUE);
		tabbedPaneLeft.addTab("Services",
				Factory.createStrippedScrollPane(this.serviceList));
		tabbedPaneLeft.addTab("Datatypes",
				Factory.createStrippedScrollPane(this.datatypeList));
		...
		leftSplitPane = Factory.createStrippedSplitPane(
				JSplitPane.VERTICAL_SPLIT, sif_overview, sif_status, 0.9f);
	...
	private JComponent buildMainRightPanel() {
		...
		return Factory.createStrippedSplitPane(JSplitPane.VERTICAL_SPLIT,
				sif_details, sif_output, 0.7f);
	}
	...
	public void activatePanel() {
		...
		sif_status.remove(1);
		sif_status.add(new JScrollPane(this.statusTable));
		sif_status.setTitle(ResourceManager
				.getString("iframe.projectstatus.active"));
		sif_status.validate();
		sif_status.repaint();
	...
	public void deactivatePanel() {
		...
		sif_status.add(this.logoComponent);
		sif_status.setTitle(ResourceManager
				.getString("iframe.projectstatus.inactive"));
		sif_status.validate();
		sif_status.repaint();
