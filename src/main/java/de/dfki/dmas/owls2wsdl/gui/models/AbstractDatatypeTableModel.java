...
public class AbstractDatatypeTableModel extends AbstractTableModel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8648483490898105735L;
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		if (rowIndex < this.getRowCount()) {
			AbstractDatatypeElement elem = this.data.get(rowIndex);

			boolean isDatatypeElem = elem.getOwlSource().equals("DATATYPE")
					|| elem.getOwlSource().equals("RESTRICTION-ON-DATATYPE");
			boolean isObjectElem = elem.getOwlSource().equals("OBJECT")
					|| elem.getOwlSource().equals("RESTRICTION-ON-OBJECT");
			int curDepth = RuntimeModel.getInstance().getProject()
					.getElementDepth();
			...
			case 1:
				if (elem.getOwlSource().equals("OBJECT"))
					return "OBJ";
				else if (elem.getOwlSource().equals("RESTRICTION-ON-OBJECT"))
					return "OBJ(R)";
				else if (elem.getOwlSource().equals("DATATYPE"))
					return "DAT";
				else if (elem.getOwlSource().equals("RESTRICTION-ON-DATATYPE"))
					return "DAT(R)";
				else
					return elem.getOwlSource();
			case 2:
				return elem.getLocalTypeName();
			case 3:
				return elem.getInheritedByLocalName();
			case 4:
				String defaultXsdType = RuntimeModel.getInstance().getProject()
						.getDefaultXsdType();
				...
				if (isObjectElem) {
					...
					if (elem.isPrimitive()) {
						...
						// In Eigenschaften Defaultwert f�r Thing (future work)
						// String xsdtypeString = "anyType";
						// if(defaultXsdType.contains("#")) {
						// xsdtypeString =
						// defaultXsdType.substring(defaultXsdType.indexOf("#")+1);
						// }
						...
						AbstractDatatype atype = AbstractDatatypeKB
								...
								.get(elem.getType());
						if (atype.getProperties(curDepth).isEmpty()
								&& atype.getXsdType() != null) {
							String xsdtype = atype.getXsdType();
							if (xsdtype.contains("#")) {
								xsdtype = xsdtype.substring(xsdtype
										.indexOf("#") + 1);
							}
							return xsdtype;
						} else if (atype.getProperties(curDepth).isEmpty()
								&& atype.getXsdType() == null) {
							String xsdtype = atype.determineXsdType(
									inheritanceBehaviour, defaultXsdType);
							if (xsdtype.contains("#")) {
								xsdtype = xsdtype.substring(xsdtype
										.indexOf("#") + 1);
							}
				...
				return "D";
			case 5:
				return new Integer(elem.getLevel());
			case 6:
	...
	public void updateModel(AbstractDatatype datatype) {
		...
		System.out.println("[i] AbstractDatatypeTableModel, Count  Elements: "
				+ this.data.size());
