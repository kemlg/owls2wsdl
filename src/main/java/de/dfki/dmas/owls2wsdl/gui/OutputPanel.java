...
public class OutputPanel extends JPanel {
	...
	final URL saveIconURL = ToolBar.class
			.getResource("/images/16x16etool/save_edit.gif");
	final URL saveAllIconURL = ToolBar.class
	...
	public OutputPanel() {
		...
		this.tabbedPane.putClientProperty(Options.EMBEDDED_TABS_KEY,
				Boolean.TRUE);
		...
		JButton saveSelectedButton = new JButton(
				ResourceManager.getString("iframe.output.save"), saveIcon);
		...
		JButton saveAllButton = new JButton(
				ResourceManager.getString("iframe.output.saveall"), saveAllIcon);
	...
	public void addOutput(AbstractDatatype curtype) {
		...
		RedirectablePrintStream redirectedOutStream = new RedirectablePrintStream(
				STD_OUT, outputTextPane, createOutAttributeSet());
		...
		this.tabbedPane.addTab(curtype.getLocalName() + ".xsd",
	...
	public void addOutput(AbstractService service) {
		...
		XsdSchemaGenerator xsdgen = RuntimeModel.getInstance().getProject()
				.buildXsdSchemaGenerator();
		System.out.println("[marshall AbstractService as WSDL] "
				+ service.toString());
	...
	public void addOWLSOutput(AbstractService aService) {
		...
		XsdSchemaGenerator xsdgen = RuntimeModel.getInstance().getProject()
				.buildXsdSchemaGenerator();
		System.out.println("[marshall AbstractService as OWL-S] "
				+ aService.toString());
		...
		try {
			...
			// Factory.createStrippedScrollPane(outputTextPane));
			this.tabbedPane.addTab(aService.getLocalFilename(),
					Factory.createStrippedScrollPane(outputTextPane));
	...
	public void saveAllTabs(File directory) {
		for (int i = 0; i < this.tabbedPane.getTabCount(); i++) {
			File exportFile = new File(directory.getPath() + File.separator
	...
	private void saveSingleTab(File exportFile, int pos) {
		System.out.println("[OutputPanel] saveSingleTab " + pos + ": "
				+ exportFile.getAbsolutePath());
		String buffer = ((JEditorPane) ((JScrollPane) this.tabbedPane
				.getComponent(pos)).getViewport().getView()).getText();
		FileWriter fw = null;
		try {
			// fw = new FileWriter(
			...
			if (fw != null) {
				...
				System.out.println("[i] " + exportFile.getAbsolutePath()
						+ " saved.");
			}
		}
	}
	...
	private File showSave2Chooser() {
		...
		// }
		System.out.println("[OutputPanel] EXPORT DIR: "
				+ OWLS2WSDLSettings.getInstance().getProperty("EXPORT_PATH"));
		System.out
				.println("[OutputPanel] TAB NAME  : "
						+ this.tabbedPane.getTitleAt(this.tabbedPane
								.getSelectedIndex()));
		...
		System.out.println("[OutputPanel] FILE to save: "
	...
	static class RedirectablePrintStream extends PrintStream {
		...
		public RedirectablePrintStream(OutputStream out, JEditorPane textPane,
				SimpleAttributeSet attributeSet) {
			super(out);
		...
		public void write(byte[] buf, int off, int len) {
			if (redirect) {
				try {
					...
					buffer.trim();
					textPane.getDocument().insertString(startOffSet, buffer,
							attributeSet);
