...
import de.dfki.dmas.owls2wsdl.core.AbstractServiceCollection;
import de.dfki.dmas.owls2wsdl.core.AbstractServiceMapper;
import de.dfki.dmas.owls2wsdl.core.Project;
import de.dfki.dmas.owls2wsdl.core.WSDLOWLSExport;
...
public class ExportWSDLFrame extends JDialog implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2563015816987398097L;
	...
	private JList<AbstractService> translatableServiceList;
	private JList<AbstractService> nonTranslatableServiceList;
	...
	private static final Dimension PREFERRED_SIZE = LookUtils.IS_LOW_RESOLUTION ? new Dimension(
			500, 640) : new Dimension(500, 980);

	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals(SET_EXPORT_PATH)) {
			try {
				doBrowse();
		...
		} else if (e.getActionCommand().equals(START_EXPORT)) {
			if (this.exportDirectory.exists()) {
				try {
					doExport();
				} catch (java.io.FileNotFoundException fnfe) {
					System.err.println("[e] FileNotFoundException: "
							+ fnfe.getMessage());
				} catch (java.io.IOException ioe) {
					System.err.println("[e] IOException: " + ioe.getMessage());
				}
			} else {
				JOptionPane.showMessageDialog(this, "Export path "
						+ this.exportDirectory.getAbsolutePath()
						+ " does not exists.\nNo WSDL files created.",
						"Export WSDL Error", JOptionPane.ERROR_MESSAGE);
			}
		} else if (e.getActionCommand().equals(CHECK_STARTBUTTON)) {
			this.checkStartExportButton();
		} else if (e.getActionCommand().equals(DISPOSE_WINDOW)) {
			this.dispose();
		} else {
			System.out.println("???");
		}
	...
	public ExportWSDLFrame() {
		...
		if (OWLS2WSDLSettings.getInstance().containsKey("EXPORT_PATH")) {
			this.exportDirectory = new File(OWLS2WSDLSettings.getInstance()
		...
		generateOWLSCheckBox = new JCheckBox(
				"reconstruct OWL-S from internal metamodel");
	...
	private JPanel buildExportSubPanel(boolean debugmode) {
		FormLayout layout = new FormLayout(
				"left:10dlu, fill:pref:grow, 10px, fill:pref");
		...
		scrollPane_1.setPreferredSize(translatableServiceList
		...
		scrollPane_2.setPreferredSize(nonTranslatableServiceList
				.getPreferredScrollableViewportSize());
		...
		layout.setRowSpec(17, new com.jgoodies.forms.layout.RowSpec(
				"fill:pref:grow"));
	...
	private JPanel buildCommandSubPanel() {
		...
		startExportButton.setActionCommand(START_EXPORT);
		startExportButton.addActionListener(this);
		startExportButton.setEnabled(false);

		JButton closeButton = new JButton("Close");
		closeButton.setActionCommand(DISPOSE_WINDOW);
	...
	public void update(Vector<AbstractService> selectedServicesList) {
		...
		for (Iterator<AbstractService> it = selectedServicesList.iterator(); it
				.hasNext();) {
			AbstractService aService = it.next();
			if (aService.istranslatable()) {
				((ServiceListModel) this.translatableServiceList.getModel())
						.addAbstractService(aService);
			} else {
				((ServiceListModel) this.nonTranslatableServiceList.getModel())
						.addAbstractService(aService);
			}
		}
		...
		this.translatableCount = Integer
				.toString(((ServiceListModel) this.translatableServiceList
						.getModel()).getSize());
		this.nontranslatableCount = Integer
				.toString(((ServiceListModel) this.nonTranslatableServiceList
						.getModel()).getSize());

		this.countInformation.setText("Translatable: " + this.translatableCount
				+ " Non-Translatable: " + this.nontranslatableCount);
	...
	private void checkStartExportButton() {
		System.out.println("Translatable services: "
				+ this.translatableServiceList.getModel().getSize());
		if (this.translatableServiceList.getModel().getSize() > 0
				&& this.exportDirectory.isDirectory()
				&& (this.generateWSDLCheckBox.isSelected() || this.generateOWLSCheckBox
						.isSelected())) {
	...
	public static void main(String[] args) {
		...
		try {
			AbstractDatatypeMapper.getInstance().loadAbstractDatatypeKB(
					new File("d:\\tmp\\KB\\KB_Student-MAP.xml"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		File f = new File("d:\\tmp\\serviceCollection1.xml");
		AbstractServiceCollection collection = AbstractServiceMapper
				.getInstance().loadAbstractServiceCollection(f);

		ef.update(collection.getServiceCollection()); // dummy to load at least
														// OWLS2WSDL settings
	}
	...
	private void doExport() throws java.io.IOException,
	...
	private void doBrowse() throws java.io.IOException {
		...
		if (!selectedDirectory.exists()) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Not a valid file path "
						+ this.exportDirectory.getAbsolutePath() + "\n" + e,
						"Error", JOptionPane.ERROR_MESSAGE);
			}
		}
