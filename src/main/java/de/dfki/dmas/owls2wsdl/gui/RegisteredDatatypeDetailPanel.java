...
import de.dfki.dmas.owls2wsdl.gui.models.AbstractDatatypeTableColummModel;
import de.dfki.dmas.owls2wsdl.gui.models.AbstractDatatypeTableModel;
import de.dfki.dmas.owls2wsdl.gui.models.RegisteredDatatypeElementTableColumnModel;
 ...
 */
public class RegisteredDatatypeDetailPanel extends JComponent implements
		ChangeListener {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4231595105102629200L;
	private JTextField datatypeUrlField;
	private JComboBox<String> datatypeRdfTypeField;
	private JComboBox<String> datatypeXsdTypeField;
	...
	private JList<String> datatypeParentList;
	private JList<String> datatypeIntersectionList;
	private JList<String> datatypeRangeList;

	private JList<AbstractDatatypeElement> datatypeMetaElementList;
	...
	public void stateChanged(ChangeEvent event) {
		System.out.println("[Tab] ChangeEvent: "
				+ dtTabs.getTitleAt(dtTabs.getSelectedIndex()));
		...
		// }
		// // else
		// if(dtTabs.getTitleAt(dtTabs.getSelectedIndex()).equals("Model")) {
		// //
	...
	private void setDefaultTypes4XsdTypeField() {
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.removeAllElements();
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#anyType");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#string");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#decimal");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#integer");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#float");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#boolean");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#date");
		((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				.addElement("http://www.w3.org/2001/XMLSchema#time");
	...
	private void initComponents() {
		...
		imageFailed = getClass().getClassLoader().getResource(
				"images/failed.gif");
		...
		datatypeRdfTypeField = new JComboBox<String>(
				new DefaultComboBoxModel<String>());
		((DefaultComboBoxModel<String>) datatypeRdfTypeField.getModel())
				.addElement("http://www.w3.org/2000/01/rdf-schema#Resource");
		...
		// AbstractDatatypeKB.getInstance().getAbstractDatatypeKBData().get(datatypeRdfTypeField.getSelectedItem().toString());
		...
		datatypeXsdTypeField = new JComboBox<String>(
				new DefaultComboBoxModel<String>());
		this.setDefaultTypes4XsdTypeField();

		Dimension defaultButtonDim1 = new Dimension(45,
				datatypeRdfTypeField.getPreferredSize().height);
		Dimension defaultButtonDim2 = new Dimension(55,
				datatypeRdfTypeField.getPreferredSize().height);
		...
		saveXsdTypeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RuntimeModel
						...
						.getSelectedDatatype()
						.setXsdType(
								datatypeXsdTypeField.getSelectedItem()
										.toString());
				updateXsdTypeInformationSettings(RuntimeModel.getInstance()
						.getSelectedDatatype());
			}
		});
		...
		removeXsdTypeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RuntimeModel.getInstance().getSelectedDatatype()
						.setXsdType(null);
				updateXsdTypeInformationSettings(RuntimeModel.getInstance()
						.getSelectedDatatype());
			}
		...
		datatypeElementTable
				.setColumnModel(new AbstractDatatypeTableColummModel());
		datatypeElementTable
				.setAutoResizeMode(JTable.AUTO_RESIZE_SUBSEQUENT_COLUMNS);
		datatypeElementTable.setPreferredScrollableViewportSize(new Dimension(
				550, 100));
		datatypeElementTable.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					public void valueChanged(ListSelectionEvent e) {
						if (!e.getValueIsAdjusting()) {
							System.out.println("[i] selectedRow: "
									+ datatypeElementTable.getSelectedRow());
							if (datatypeElementTable.getSelectedRow() >= 0) {
								System.out.println("ValueIsAdjusting: "
										+ ((AbstractDatatypeTableModel) datatypeElementTable
												.getModel()).getValueAt(
												datatypeElementTable
														.getSelectedRow(),
												datatypeElementTable
														.getSelectedColumn()));
								AbstractDatatypeElement elem = (AbstractDatatypeElement) ((AbstractDatatypeTableModel) datatypeElementTable
										.getModel())
										.getAbstractDatatypeElementAt(datatypeElementTable
												.getSelectedRow());
								((RegisteredDatatypeElementTableModel) elementRestrictionTable
										.getModel()).updateModel(elem);
								elementRestrictionTable.revalidate();
								elementRestrictionTable.updateUI();
							} else {
		...
		datatypeElementTable.getSelectionModel().setSelectionMode(
				ListSelectionModel.SINGLE_SELECTION);

		final DefaultListModel<String> parentInformation = new DefaultListModel<String>();
		datatypeParentList = new JList<String>(parentInformation);
		datatypeParentList.setVisibleRowCount(4);
		// datatypeParentList.setBorder(BorderFactory.createLineBorder(Color.GRAY));

		final DefaultListModel<String> intersectionInformation = new DefaultListModel<String>();
		datatypeIntersectionList = new JList<String>(intersectionInformation);
		...
		datatypeMetaElementList = new JList<AbstractDatatypeElement>(
				model_metaelements);
		datatypeMetaElementList.setVisibleRowCount(4);
		datatypeMetaElementList
				.addListSelectionListener(new ListSelectionListener() {
					public void valueChanged(ListSelectionEvent e) {
						if (!e.getValueIsAdjusting()) {
							System.out.println(">>>> "
									+ ((AbstractDatatypeMetaElementListModel) datatypeMetaElementList
											.getModel())
											.getElementAt(datatypeMetaElementList
													.getSelectedIndex()));
		...
		final DefaultListModel<String> rangeListModel = new DefaultListModel<String>();
		datatypeRangeList = new JList<String>(rangeListModel);
		...
		elementRestrictionTable = new JTable(restrictionTableModel,
				new RegisteredDatatypeElementTableColumnModel());
		elementRestrictionTable
				.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);
		elementRestrictionTable
				.setPreferredScrollableViewportSize(new Dimension(500, 60));
		elementRestrictionTable.getSelectionModel().setSelectionMode(
				ListSelectionModel.SINGLE_SELECTION);
		elementRestrictionTable.setEnabled(false);
		elementRestrictionTable.getColumnModel().getColumn(0).setWidth(100);
		elementRestrictionTable.getColumnModel().getColumn(1)
				.setPreferredWidth(400);
		// elementRestrictionTable.getColumnModel().addColumnModelListener(new
	...
	private void buildPanel() {
		...
		FormLayout layout = new FormLayout(
				"right:pref, 10px, fill:pref:grow, 2dlu, right:pref",
		...
		this.add(
				DefaultComponentFactory.getInstance().createSeparator(
						ResourceManager.getString("type.sep.info")),
				cc.xyw(1, 1, 5));
		...
		dtTabs.add("Intersection Classes", new JScrollPane(
				datatypeIntersectionList));
		...
		this.add(
				DefaultComponentFactory.getInstance().createSeparator(
						ResourceManager.getString("type.sep.range")),
				cc.xyw(1, 11, 5));
	 ...
	 * @return found flag, shows if something other than the default xsd type
	 *         found.
	 */
	private boolean updateXsdTypeFromRdfType(AbstractDatatype atype,
		...
		for (int i = 0; i < datatypeRdfTypeField.getItemCount(); i++) {
			System.out.println("[i] process rdf:type "
					+ datatypeRdfTypeField.getItemAt(i));
			...
			if (atype.getTypeList().size() == 1) {
				...
				((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
						.setSelectedItem(xsdTypeString);
			} else if (atype.getTypeList().size() > 1) {
				String rdfTypeString = datatypeRdfTypeField.getSelectedItem()
						.toString();
				if (AbstractDatatypeKB.getInstance()
						.getAbstractDatatypeKBData().get(rdfTypeString)
						.hasXsdType()) {
					xsdTypeString = AbstractDatatypeKB.getInstance()
							.getAbstractDatatypeKBData().get(rdfTypeString)
							.getXsdType();
				}
				((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
				...
				((DefaultComboBoxModel<String>) datatypeXsdTypeField.getModel())
						.setSelectedItem(DEFAULT_XSDTYPE);
			}
	...
	private boolean updateXsdTypeFromParent(AbstractDatatype atype,
			String DEFAULT_XSDTYPE) {
		boolean found = false;
	...
	private void updateXsdTypeInformationSettings(AbstractDatatype atype) {
		...
		final String DEFAULT_XSDTYPE = RuntimeModel.getInstance().getProject()
				.getDefaultXsdType();
		...
		if (atype.hasXsdType()) {
			...
			String inheritanceBehaviour = RuntimeModel.getInstance()
					.getProject().getTypeInheritanceBehaviour();
			System.out.println("TypeInheritanceBehaviour: "
	...
	public void updatePanel(Object obj) {
		...
		if (atype.getTypeList().isEmpty()) {
			...
			((DefaultComboBoxModel<String>) datatypeRdfTypeField.getModel())
					.removeAllElements();
			for (Iterator<String> it = atype.getTypeList().iterator(); it
					.hasNext();) {
				((DefaultComboBoxModel<String>) datatypeRdfTypeField.getModel())
		...
		DefaultListModel<String> parentInformation = (DefaultListModel<String>) datatypeParentList
				.getModel();
		parentInformation.removeAllElements();
		for (Iterator<String> it = atype.getParentList().iterator(); it
				.hasNext();) {
		...
		DefaultListModel<String> intersectionInformation = (DefaultListModel<String>) datatypeIntersectionList
				.getModel();
		intersectionInformation.removeAllElements();
		for (Iterator<String> it = atype.getIntersectionList().iterator(); it
				.hasNext();) {
		...
		DefaultListModel<String> rangeListModel = (DefaultListModel<String>) datatypeRangeList
				.getModel();
		rangeListModel.removeAllElements();
		for (Iterator<String> it = atype.getIndividualRange().keySet()
				.iterator(); it.hasNext();) {
