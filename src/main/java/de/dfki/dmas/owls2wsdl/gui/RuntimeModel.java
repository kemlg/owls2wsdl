...
/**
 * Singleton Observable Model descripes GUI status: for SERVICE_VIEW and
 * DATATYPE_VIEW
 * 
...
public class RuntimeModel extends Observable {
	...
	public Vector<String> getOWLSURLDialogItems() {
		return (Vector<String>) this.status.get("OWLS_URLDialogItems");
	}

	@SuppressWarnings("unchecked")
	public Vector<String> getOWLURLDialogItems() {
		return (Vector<String>) this.status.get("OWL_URLDialogItems");
	}
	...
	public AbstractService getSelectedService() {
		...
		if (this.status.containsKey("SELECTED_SERVICE")) // ||
															// this.status.containsKey(RuntimeModel.SINGLE_SERVICE_SELECTED))
	...
	public void printStatus() {
		...
		while (it.hasNext()) {
			...
			System.out.println("[RuntimeModel] " + tempkey + "=("
					+ status.get(tempkey).toString() + ")");
		}
	}
}
