...
public class RegisteredDatatypeOverviewPanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2625676772401465223L;

	private AbstractDatatypeListModel model;

	private JList<AbstractDatatype> typeList;
	...
	private void initComponents() {
		...
		typeList = new JList<AbstractDatatype>(model);
		...
		typeList.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					detailViewPanel
							.updatePanel(((AbstractDatatypeListModel) typeList
									.getModel()).getElementAt(typeList
									.getSelectedIndex()));
				}
			}
		});

		typeList.getSelectionModel().setSelectionMode(
