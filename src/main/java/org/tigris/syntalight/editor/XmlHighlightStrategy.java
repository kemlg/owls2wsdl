...
class XmlHighlightStrategy extends HighlightStrategy {
	...
	public XmlHighlightStrategy(SyntaxDocument doc) {
		...
		MutableAttributeSet normalAttributes = getAttributes(Color.black, true,
				false);
		MutableAttributeSet blockCommentAttributes = getAttributes(new Color(0,
				128, 0), false, true);
		MutableAttributeSet xslTagAttributes = getAttributes(Color.blue, true,
				false);
		MutableAttributeSet xmlTagAttributes = getAttributes(Color.black,
				false, false);
		MutableAttributeSet xslAliasTagAttributes = getAttributes(Color.blue,
				false, false);
		MutableAttributeSet wsdlTagAttributes = getAttributes(Color.red, true,
				false);
		MutableAttributeSet schemaTagAttributes = getAttributes(Color.BLUE,
				false, false);
		MutableAttributeSet soapTagAttributes = getAttributes(Color.pink, true,
				false);
		MutableAttributeSet processorInstructionAttributes = getAttributes(
				Color.lightGray, false, false);

		setNormalAttributes(normalAttributes);

		addAttributeSetForToken(BlockCommentTokenType.class,
				blockCommentAttributes);
		addAttributeSetForToken(ProcessorInstructionTokenType.class,
				processorInstructionAttributes);
		...
		addAttributeSetForToken(XslAliasTagTokenType.class,
				xslAliasTagAttributes);
	...
	private MutableAttributeSet getAttributes(Color color, boolean bold,
			boolean italic) {
