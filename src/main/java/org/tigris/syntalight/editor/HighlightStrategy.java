...
public class HighlightStrategy {
	...
	private Map<Class<?>, AttributeSet> attributeMapByTokenType = new HashMap<Class<?>, AttributeSet>();
	...
	public void highlight() throws BadLocationException {
		do {
			...
			int startOffset = 0;
			Lazer tokenizer = new Lazer(languageStrategy, content,
					Lazer.COMPOUND | Lazer.APPEND_WHITESPACE);
			Token token = null;
			System.out.println("Highlighting with "
					+ languageStrategy.getClass().getName());
			System.out.println("Document " + doc);
			while (tokenizer.hasMoreElements()) {
				token = (Token) tokenizer.nextElement();
				AttributeSet as = (AttributeSet) attributeMapByTokenType
						.get(token.getType().getClass());
				if (as == null)
					as = normalAttributes;
				if (doc.isRestartHighlightingRequested())
					break;
				doc.setCharacterAttributes(startOffset, token.getLength(), as,
						false);
	...
	protected void addAttributeSetForToken(Class<?> tokenType,
