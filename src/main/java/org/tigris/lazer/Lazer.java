 ...
 * @author Bob Tarling
 */
public class Lazer implements Enumeration<Token> {
	...
	public static int COMPOUND_WHITESPACE = 16; // TODO - need a better solution
												// for this.
	 ...
	 */
	public Lazer(TokenList<Token> tokenList, int mode, int position) {
	}
	...
	private boolean shouldCompound(int mode, Token nextToken, Token returnToken) {
		return ((mode & COMPOUND) == COMPOUND && nextToken != null && returnToken
				.getType() == nextToken.getType());
	}

	private boolean shouldAppendWhitespace(int mode, Token nextToken,
			Token returnToken) {
		return ((mode & APPEND_WHITESPACE) == APPEND_WHITESPACE
				&& nextToken != null && nextToken.getType() == WhiteSpaceTokenType
				.getInstance());
	}
	...
	private boolean shouldIgnore(Token token) {
		if (token.getType() instanceof WhiteSpaceTokenType
				&& (mode & IGNORE_WHITESPACE) == IGNORE_WHITESPACE)
			return true;
		if (token.getType() instanceof CommentTokenType
				&& (mode & IGNORE_COMMENTS) == IGNORE_COMMENTS)
			return true;
		return false;
	}
}
