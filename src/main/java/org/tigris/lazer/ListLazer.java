 ...
 * @author Bob Tarling
 */
public class ListLazer implements Enumeration<Token> {
	...
	/** Creates a new instance of LanguageTokenizer */
	public ListLazer(List<Token> tokens) {
		this(tokens, 0, 0);
	}

	/** Creates a new instance of LanguageTokenizer */
	public ListLazer(List<Token> tokens, int mode) {
		this(tokens, mode, 0);
	}

	/** Creates a new instance of LanguageTokenizer */
	public ListLazer(List<Token> tokens, int mode, int position) {
	...
	private Token getNextToken() {
		...
		if (compoundWhiteSpace
				&& nextToken.getType() instanceof WhiteSpaceTokenType) {
			while (position < tokens.size()
					&& ((Token) (tokens.get(position))).getType() instanceof WhiteSpaceTokenType) {
