...
public abstract class AbstractTokenFactory {
	 ...
	 */
	protected static Map<Object, Token> previousTokens = new HashMap<Object, Token>();
	/**
	 * If true then the tokens returned will contain the text of the token. Some
	 * clients do not need to know the contents of the token, just the length.
	 * Setting this to false gives some small performance gain by lower object
	 * instantiation.
	 */
	private boolean storeText = true;
	/**
	 * If the reuse flag is set then the flyweight pattern is abandoned and the
	 * factory simply returns the same token populated with different data each
	 * time. This can only be used for performance or memory resource situations
	 * where the client code does not require the Token to be immutable.
	 */
	 ...
	 * 
	 * @param text
	 *            The string from which to extract the token
	 * @param start
	 *            The start position of the token in the text
	 * @param end
	 *            The end position of the token in the text
	 * @param tokenType
	 *            The type to assign to the token
	 * @return the token
	 */
	public Token createToken(String text, int start, int end,
			TokenType tokenType) {
		if (storeText) {
	...
	protected int indexOf(String text, String searchString, int start,
			int offset) {
