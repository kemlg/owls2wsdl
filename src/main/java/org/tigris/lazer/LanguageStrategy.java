 ...
 * token factories in their constructor. Each of these factories is scanned in
 * order to determine if it is applicable for the current text and called if so.
 * 
 * @author Bob Tarling
 */
public abstract class LanguageStrategy {

	protected Collection<AbstractTokenFactory> tokenFactories = new ArrayList<AbstractTokenFactory>();
	...
	public Token createToken(String text, int start) {
		...
		Iterator<AbstractTokenFactory> it = tokenFactories.iterator();
		while (it.hasNext()) {
			AbstractTokenFactory factory = it.next();
