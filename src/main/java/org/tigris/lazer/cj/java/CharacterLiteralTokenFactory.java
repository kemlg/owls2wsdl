 ...
 * @author Bob Tarling
 */
public class CharacterLiteralTokenFactory extends AbstractTokenFactory {

	private static char escape = '\\';
	private static char literalQuote = '\'';

	/** Creates a new instance of BlockCommentTokenFactory */
	public CharacterLiteralTokenFactory() {
	}
	...
	public Token createToken(String text, int start) {
		...
		while (position < text.length()
				&& (c = text.charAt(position++)) != quote) {
			if (c == escape)
				++position;
		}
		return createToken(text, start, position,
				StringLiteralTokenType.getInstance());
	}
}
